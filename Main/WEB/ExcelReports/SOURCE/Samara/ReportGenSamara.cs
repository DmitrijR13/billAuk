...
namespace STCLINE.KP50.REPORT
{
    public partial class ReportGen
    {
        ...
        {
            StreamWriter sw = new StreamWriter(@"C:\temp\SetReportSing.txt", false);
            sw.WriteLine("1");
            //excelL.ExlWs.PageSetup.RightFooter = "&P";
            //if (!Interfaces.Points.IsSmr) return;
            sw.WriteLine("arow = " + arow);
            ...
            if (1 == 1)
            {
                ...
                excell.Merge(Type.Missing);
                excell.Value2 = "Гордиевский П.А.";
                //excell.Value2 = "Аханьков С.Л.";
            ...
            //    excell.Value2 = uname.Rows.Count == 1 ? Utils.GetCorrectFIO(uname.Rows[0]["uname"].ToString().Trim()) : "";
            //    conn_web.Close();
            //}
            //else
            //{
        ...
        private string SignNames(int nzp_prm)
        {
            ...
            try
            {
                ...
                switch (nzp_prm)
                {
                    case 1291: return principal.Rows.Count == 1 ? principal.Rows[0]["val_prm"].ToString().Trim() : "Ген. директор";
                    case 1292: return principal.Rows.Count == 1 ? principal.Rows[0]["val_prm"].ToString().Trim() : "Исполнитель";
                    case 1293: return principal.Rows.Count == 1 ? principal.Rows[0]["val_prm"].ToString().Trim() : "";
                    case 1294: return principal.Rows.Count == 1 ? principal.Rows[0]["val_prm"].ToString().Trim() : "Гордиевский П.А.";
                    case 1295: return principal.Rows.Count == 1 ? principal.Rows[0]["val_prm"].ToString().Trim() : "Волков С.Б.";
                    case 1296: return principal.Rows.Count == 1 ? principal.Rows[0]["val_prm"].ToString().Trim() : "";
                }
            }
            catch (Exception ex)
            {
                ...
                switch (nzp_prm)
                {
                    ...
                    case 1292: return "Исполнитель";
                    case 1293: return "";
                    case 1294: return "Гордиевский П.А.";
                    case 1295: return "Волков С.Б.";
                    case 1296: return "";
                }
            }
            finally
        ...
        public Returns LsKvarDomParam(List<Prm> listprm, int nzpUser, ref string fileName)
        {
            ...
            //создание dataTable
            #region Cоздание dataTable

            var exR = new ExcelRep();
            System.Data.DataTable dt = exR.GetDataReportGenertor(out ret, Convert.ToString(nzpUser));
            exR.Close();
            ...
            try
            {
                ...
                var columnsNames = (from c in listCol 
                                    where c.ColumnName.Contains("val") 
                                    select c.ColumnName.Substring(c.ColumnName.IndexOf('_') + 1)).ToList<string>();
                var parNames = (from pn in listprm 
                                where !columnsNames.Contains(Convert.ToString(pn.nzp_prm)) 
                                select pn.name_prm).ToList<string>();

                foreach (string newName in parNames) dt.Columns.Add(newName);
            }
            catch (Exception ex)
            {
            ...
            var eformat = new ExcelFormater();

            //Массив заголовков
            var headerData = new object[dt.Columns.Count];
            //Массив форматов
            var typeHeader = new string[headerData.Length];
            ...
            try
            {
                ...
                //Создаем тело
                if (ret.result && dt.Rows.Count < 65001) ret = excelCr.MakeBody(dt, 2, 0, ref excelL.ExlWs);

                //Стандартный формат
                if (ret.result) ret = excelL.ApplyStandartFormat(dt.Rows.Count, 3, headerData.Length, typeHeader);

                #endregion

                #region Надпись "Нет данных" если данных нет)

                if (dt.Rows.Count == 0) excelCr.MakeName("Нет данных", "A4", dt.Columns.Count, 6, ref excelL.ExlWs);
                ...
                {
                    //имя файла                                                         
                    fileName = GetFileName(Constants.ExcelDir, "SpLs_" + DateTime.Now.GetHashCode() + nzpUser) + ".xls";

                    //Сохранение
                    ret = excelL.SaveFile(Constants.ExcelDir + fileName, fileName, ref excelL.ExlWb, ref excelL.ExlApp);
                    if (InputOutput.useFtp) fileName = InputOutput.SaveOutputFile(Constants.ExcelDir + fileName);
            ...
            {
                //удаление объекта
                if (excelL != null) excelL.DeleteObject();
            }
        ...
        public Returns GetSostGilFond(Prm prm_, string Nzp_user, ref string fileName)
        {
            ...
            try
            {
                ...
                if (DT != null)
                {
                    ...
                    excells1.EntireColumn.NumberFormatLocal = "# ##0,00";
            ...
            try
            {
                ...
                if (dt != null)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        excells1 = excelL.ExlWs.get_Range("C7", "C" + (excelRow + 8).ToString());
                        excells1.NumberFormatLocal = "# ##0,00";

                        excells1 = excelL.ExlWs.get_Range("G7", "N" + (excelRow + 8).ToString());
                        excells1.NumberFormatLocal = "# ##0,00";
                        #endregion
                        ...
                        excells1 = excelL.ExlWs.get_Range("G7", "S" + (excelRow + 8).ToString());
                        excells1.NumberFormatLocal = "# ##0,00";
                    }
                    else
                    {
        ...
        public Returns GetSpravSodergOdn9(Prm prm_, string Nzp_user, ref string fileName)
        {
            ...
            //StreamWriter sw = new StreamWriter(@"C:\temp\GetSpravSodergOdn9.txt", false);
            streets.Add(" ГЕОРГИЯ МИТИРЕВА", " ГЕОРГИЯ МИТИРЕВА ПРОЕЗД");
            streets.Add(" САДОВЫЙ", " САДОВЫЙ ПРОЕЗД");
            ...
            try
            {
                ...
                if (dt != null)
                {
                    ...
                    while (i < dt.Rows.Count)
                    {
                        ...
                        string ulicaTemp = dt.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", "");
                        // sw.WriteLine(ulicaTemp);
                        if (streets.ContainsKey(ulicaTemp))
                        {
                            ulica = streets[dt.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", "")].Trim();
                            //sw.WriteLine("find = " + ulica);
                        }
                        else
                        {
                            ulica = dt.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", "").Trim();
                            //sw.WriteLine("Not find = " + ulica);
                        }
                        //ulica = dt.Rows[i]["ulica"].ToString().Trim();

                        string domTemp = dt.Rows[i]["ndom"].ToString().Trim();
                        int pos = 0;
                        int isNum;
                        foreach (char c in domTemp)
                        {
                        ...
                        dom = domTemp.Substring(0, pos).Trim();
                        litera = domTemp.Substring(pos).Trim();
        ...
        public Returns GetSpravPoOtklUslug(int nzp_user, int nzp_serv, int m, int y, ref string fileName)
        {
            ...
            {
                MonitorLog.WriteLog("Ошибка11111 вставки строк итого GetSpravPoOtklUslug : " + ex.Message, MonitorLog.typelog.Error, true);
                ret.result = false;
                ret.text = ex.Message;
                return ret;
            }
            ...
            string[] HeaderData = new string[] { "Поставщики к/услуг", "Виды услуг", "ЖЭУ","Адрес" ,"Кол.Квар", 
                                                 "Кол.квар/дней", "Кол.квар/часов", "Кол. жил.", "Сумма возврата" };
            //создаем массив форматов
            string[] TypeHeader = new string[] { "char", "char", "char", "char", "int",
                                                 "int", "int", "int", "float"};
            //футер
            string[] footer_data = null;
            ...
            try
            {
                ...
                if (DT != null)
                {
                    ...
                    try
                    {
                        ...
                        fileName = GetFileName(STCLINE.KP50.Global.Constants.ExcelDir, "SpravGroupSoderg000" +
                            prm_.year_.ToString() + "_" +
                            prm_.month_.ToString() + "_" + Nzp_user) + ".xls";
                        ret = ExcelL.SaveFile(STCLINE.KP50.Global.Constants.ExcelDir + fileName, fileName, ref ExcelL.ExlWb, ref ExcelL.ExlApp);
                        if (InputOutput.useFtp)
            ...
            try
            {
                if (DT != null)
                {
                    ...
                    while (i < DT.Rows.Count)
                    {
                        ...
                        excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlLeft;
                        excells1.Merge(Type.Missing);
                        sw.WriteLine(DT.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", ""));
                        if (streets.ContainsKey(DT.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", "")))
                            excells1.FormulaR1C1 = streets[DT.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", "")].Trim();
                        else
                            excells1.FormulaR1C1 = DT.Rows[i]["ulica"].ToString().Trim().Replace("САМАРА Г,", "").Trim();

                        excells1 = ExcelL.ExlWs.get_Range("D" + ExcelRow.ToString(), "D" + ExcelRow.ToString());
                        excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
                        excells1.Merge(Type.Missing);
                        string dom = DT.Rows[i]["ndom"].ToString().Trim();
                        int pos = 0;
                        int isNum;
                        foreach (char c in dom)
                        {
                            bool result = Int32.TryParse(c.ToString(), out isNum);
                            if (result)
                                pos++;
                        ...
                        excells1.FormulaR1C1 = dom.Substring(0, pos).Trim();

                        excells1 = ExcelL.ExlWs.get_Range("E" + ExcelRow.ToString(), "E" + ExcelRow.ToString());
                        excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
                        excells1.FormulaR1C1 = dom.Substring(pos).Trim();
            ...
            try
            {
                if (DT != null)
                {
                    ...
                    while (i < DT.Rows.Count)
                    {
                        ...
                        excells1.NumberFormat = "# ##0,00";
                        excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["rsum_tarif"]) + Convert.ToDecimal(DT.Rows[i]["reval"]);
                        ...
                        if (DT.Rows[i]["rsum_tarif"] != DBNull.Value)                           
                        {
                            ...
                            itogString[16] = itogString[16] + System.Convert.ToDecimal(DT.Rows[i]["rsum_tarif"]);
                        }
                        ...
                        if (DT.Rows[i]["reval"] != DBNull.Value)                           
                        {
                            ...
                            itogString[16] = itogString[16] + System.Convert.ToDecimal(DT.Rows[i]["reval"]);
                        }
                        if (DT.Rows[i]["vol_charge_kub"] != DBNull.Value)
                    ...
                    excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
                    excells1.NumberFormatLocal = "# ##0,0000";
            ...
            try
            {
                if (DT != null)
                {
                    ...
                    while (i < DT.Rows.Count)
                    {
                            ...
                            excells1 = ExcelL.ExlWs.get_Range("H" + ExcelRow.ToString(), "H" + ExcelRow.ToString());
                            if (DT.Rows[i]["volume_all_kub"].ToString() != "0.00")
                                excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["volume_all_kub"]) + Convert.ToDecimal(DT.Rows[i]["vol_odn_kub"]);

                            excells1 = ExcelL.ExlWs.get_Range("I" + ExcelRow.ToString(), "I" + ExcelRow.ToString());
                            if (DT.Rows[i]["volume_all_gkal"].ToString() != "0.0000")
                                excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["volume_all_gkal"]) - Convert.ToDecimal(DT.Rows[i]["vol_odn_gkal"]);

                            excells1 = ExcelL.ExlWs.get_Range("J" + ExcelRow.ToString(), "J" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vol_ipu_kub"] != DBNull.Value ? DT.Rows[i]["vol_ipu_kub"] : "0";

                            excells1 = ExcelL.ExlWs.get_Range("K" + ExcelRow.ToString(), "K" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vol_ipu_gkal"] != DBNull.Value ? DT.Rows[i]["vol_ipu_gkal"] : "0";

                            excells1 = ExcelL.ExlWs.get_Range("L" + ExcelRow.ToString(), "L" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vol_npu_kub"] != DBNull.Value ? DT.Rows[i]["vol_npu_kub"] : "0";

                            excells1 = ExcelL.ExlWs.get_Range("M" + ExcelRow.ToString(), "M" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["vol_npu_gkal"]);

                            excells1 = ExcelL.ExlWs.get_Range("N" + ExcelRow.ToString(), "N" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vol_odn_kub"] != DBNull.Value ? DT.Rows[i]["vol_odn_kub"] : "0";

                            excells1 = ExcelL.ExlWs.get_Range("O" + ExcelRow.ToString(), "O" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vol_odn_gkal"] != DBNull.Value ? DT.Rows[i]["vol_odn_gkal"] : "0";
                            ...
                            excells1 = ExcelL.ExlWs.get_Range("T" + ExcelRow.ToString(), "T" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vozv_odn_kub"] != DBNull.Value ? DT.Rows[i]["vozv_odn_kub"] : "0";

                            excells1 = ExcelL.ExlWs.get_Range("U" + ExcelRow.ToString(), "U" + ExcelRow.ToString());
                            excells1.FormulaR1C1 = DT.Rows[i]["vozv_odn_gkal"] != DBNull.Value ? DT.Rows[i]["vozv_odn_gkal"] : "0";
                            ...
                            if (DT.Rows[i]["vol_charge_gkal"].ToString() != "0.0000")
                                excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["volume_all_gkal"]) - ((DT.Rows[i]["vol_odn_gkal"] != null) && DT.Rows[i]["vol_odn_gkal"].ToString() != "" ? Convert.ToDecimal(DT.Rows[i]["vol_odn_gkal"]) : 0) -
                                    ((DT.Rows[i]["vozv_odn_gkal"] != null) && DT.Rows[i]["vozv_odn_gkal"].ToString() != "" ? Convert.ToDecimal(DT.Rows[i]["vozv_odn_gkal"]) : 0) +
                                    ((DT.Rows[i]["reval_gkal"] != null) && DT.Rows[i]["reval_gkal"].ToString() != "" ? Convert.ToDecimal(DT.Rows[i]["reval_gkal"]) : 0);

                            excells1 = ExcelL.ExlWs.get_Range("AC" + ExcelRow.ToString(), "AC" + ExcelRow.ToString());
                            excells1.NumberFormat = "# ##0,00";
                        if (prm_.nzp_serv == 513)
                        {
                            excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["rsum_tarif"]) -
                                                   Convert.ToDecimal(DT.Rows[i]["sum_nedop"]) +
                                                   Convert.ToDecimal(DT.Rows[i]["reval"]);
                        }
                        else
                        {
                            excells1.FormulaR1C1 = DT.Rows[i]["sum_charge"];
                        ...
                        if (DT.Rows[i]["volume_all_gkal"] != DBNull.Value)
                            itogString[4] = itogString[4] + (Convert.ToDecimal(DT.Rows[i]["volume_all_gkal"]) - Convert.ToDecimal(DT.Rows[i]["vol_odn_gkal"]));
                            if (DT.Rows[i]["vol_ipu_kub"] != DBNull.Value)
                            ...
                            if (DT.Rows[i]["vol_odn_kub"] != DBNull.Value)
                            {
                                ...
                                itogString[3] = itogString[3] + System.Convert.ToDecimal(DT.Rows[i]["vol_odn_kub"]);
                            }

                            if (DT.Rows[i]["vol_odn_gkal"] != DBNull.Value)
                            {
                                itogString[10] = itogString[10] + System.Convert.ToDecimal(DT.Rows[i]["vol_odn_gkal"]);
                                //itogString[8] = itogString[8] - System.Convert.ToDecimal(DT.Rows[i]["vol_odn_gkal"]);
                            }
                            ...
                            if (DT.Rows[i]["vol_charge_gkal"] != DBNull.Value)
                                itogString[23] = itogString[23] + (Convert.ToDecimal(DT.Rows[i]["volume_all_gkal"]) - ((DT.Rows[i]["vol_odn_gkal"] != null) && DT.Rows[i]["vol_odn_gkal"].ToString() != "" ? Convert.ToDecimal(DT.Rows[i]["vol_odn_gkal"]) : 0) -
                                    ((DT.Rows[i]["vozv_odn_gkal"] != null) && DT.Rows[i]["vozv_odn_gkal"].ToString() != "" ? Convert.ToDecimal(DT.Rows[i]["vozv_odn_gkal"]) : 0) +
                                    ((DT.Rows[i]["reval_gkal"] != null) && DT.Rows[i]["reval_gkal"].ToString() != "" ? Convert.ToDecimal(DT.Rows[i]["reval_gkal"]) : 0));
                            if (DT.Rows[i]["sum_charge"] != DBNull.Value)
                                itogString[24] = itogString[24] + (Convert.ToDecimal(DT.Rows[i]["rsum_tarif"]) -
                                                   Convert.ToDecimal(DT.Rows[i]["sum_nedop"]) +
                                                   Convert.ToDecimal(DT.Rows[i]["reval"]));
                            if (DT.Rows[i]["odpu_kub"] != DBNull.Value)
        ...
        public void MakeListSpravSuppNachHar(System.Data.DataTable DT, ExcelLoader ExcelL, int numList, Prm prm, string nzp_user)
        {
            ...
            if (ExcelRow > 6)
            {
                ...
                excells1 = ExcelL.ExlWs.get_Range("E7", "E" + ExcelRow.ToString());
                excells1.Borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous;
                excells1.EntireColumn.NumberFormatLocal = "# ##0";
                excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
        ...
        public void GetSpravHasDolg(object container)
        {
            ...
            //Старт формирования
            ret = this.GetSpravHasDolg(cont.finder.pref, cont.nzp_kvar.ToString(), cont.yy, cont.mm, cont.nzp_user.ToString(), ref fileName);
            if (ret.result)
            {
                MonitorLog.WriteLog("Пользователь: " + cont.nzp_user + "Формирование отчета успешно завершено. ", MonitorLog.typelog.Info, true);
                //Запись в БД об успешном формировании
            ...
            try
            {
                if (DT != null)
                {
                    ...
                    while (i < DT.Rows.Count)
                    {
                        ...
                        excells1 = ExcelL.ExlWs.get_Range("Q" + ExcelRow.ToString(), "Q" + ExcelRow.ToString());
                        excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["reval_gkal"]) * (-1);

                        //sw.WriteLine(DT.Rows[i]["reval_gkal"].ToString());
                        //sw.WriteLine(DT.Rows[i]["reval"].ToString());

                        excells1 = ExcelL.ExlWs.get_Range("R" + ExcelRow.ToString(), "R" + ExcelRow.ToString());
                        if (DT.Rows[i]["reval"].ToString() != "0.00")
                            excells1.FormulaR1C1 = Convert.ToDecimal(DT.Rows[i]["reval"]) * (-1);
                        ...
                        if (DT.Rows[i]["sum_nedop"] != DBNull.Value)
                            itogString[11] = itogString[11] + System.Convert.ToDecimal(DT.Rows[i]["sum_nedop"]);
                        if (DT.Rows[i]["reval_gkal"] != DBNull.Value)
                            itogString[12] = itogString[12] + (System.Convert.ToDecimal(DT.Rows[i]["reval_gkal"]) * (-1));
                        if (DT.Rows[i]["reval"] != DBNull.Value)
                            itogString[13] = itogString[13] + (System.Convert.ToDecimal(DT.Rows[i]["reval"]) * (-1));
                    ...
                    excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
                    excells1.NumberFormatLocal = "# ##0,00";
                    excells1 = ExcelL.ExlWs.get_Range("S13", "S" + ExcelRow.ToString());
                    excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
                    excells1.NumberFormatLocal = "# ##0,0000";
                    excells1 = ExcelL.ExlWs.get_Range("T13", "T" + ExcelRow.ToString());
                    excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
        ...
        public Returns GetSpravSoderg7(Prm prm_, string Nzp_user, ref string fileName)
        {
            ...
            string name_supp = "";
            Dictionary<string, string> streets = new Dictionary<string, string>();
            //StreamWriter sw = new StreamWriter(@"C:\temp\GetSpravSoderg7.txt", false);
            streets.Add(" ГЕОРГИЯ МИТИРЕВА", " ГЕОРГИЯ МИТИРЕВА ПРОЕЗД");
            streets.Add(" САДОВЫЙ", " САДОВЫЙ ПРОЕЗД");
            streets.Add(" МАЛЬЦЕВА", " МАЛЬЦЕВА ПРОЕЗД");
            //создание dataTable
                ...
                if (DT != null)
                {
                    ...
                    try
                    {
                        ...
                        fileName = GetFileName(STCLINE.KP50.Global.Constants.ExcelDir,
                            prm_.month_.ToString() + "_Дислокация_Водоотведение_" +
                             "_" + Nzp_user) + ".xls";
                        ret = ExcelL.SaveFile(Global.Constants.ExcelDir + fileName, fileName, ref ExcelL.ExlWb, ref ExcelL.ExlApp);
                        ExcelL.ExlWb.Close();
                        if (InputOutput.useFtp)
                    ...
                    if (DT.Rows.Count > 0)
                    {
                        ...
                        while (i < DT.Rows.Count)
                        {
                            ...
                            while ((i < DT.Rows.Count) & (old_dom == DT.Rows[i]["nzp_dom"].ToString()))  // 
                            {
                                ...
                                ExcelL.ExlWs.Cells[ExcelRow + 5, ExcelCol] = Decimal.Parse(DT.Rows[i]["rsum_tarif"].ToString()) - Decimal.Parse(DT.Rows[i]["sum_nedop"].ToString())
                                    - Decimal.Parse(DT.Rows[i]["reval_k"].ToString()) + Decimal.Parse(DT.Rows[i]["reval_d"].ToString());

                                sum_nedop = sum_nedop + Decimal.Parse(DT.Rows[i]["sum_nedop"].ToString());
                                sum_odn = sum_odn + Decimal.Parse(DT.Rows[i]["sum_odn"].ToString());
                                sum_charge = sum_charge + Decimal.Parse(DT.Rows[i]["rsum_tarif"].ToString()) - Decimal.Parse(DT.Rows[i]["sum_nedop"].ToString())
                                    - Decimal.Parse(DT.Rows[i]["reval_k"].ToString()) + Decimal.Parse(DT.Rows[i]["reval_d"].ToString());
                                rsum_tarif = rsum_tarif + Decimal.Parse(DT.Rows[i]["rsum_tarif"].ToString());
                                reval_d = reval_d + Decimal.Parse(DT.Rows[i]["reval_d"].ToString());
                                reval_k = reval_k + Decimal.Parse(DT.Rows[i]["reval_k"].ToString());
                        ...
                        excells1 = ExcelL.ExlWs.get_Range("C7", "C" + (ExcelRow + 6).ToString());
                        excells1.NumberFormatLocal = "# ##0,00";

                        excells1 = ExcelL.ExlWs.get_Range("G7", "S" + (ExcelRow + 6).ToString());
                        excells1.NumberFormatLocal = "# ##0,00";
                    }
                    else
                    {
        ...
        public Returns GetDolgSpis(Prm prm_, string Nzp_user, ref string fileName)
        {
            ...
            //StreamWriter sw = new StreamWriter(@"C:/temp/dolgSpis.txt", false);
            ExcelLoader ExcelL = null;
            try
            {
                ExcelL = new ExcelLoader();
                ...
                if (DT != null)
                {
                    ...
                    while (i < DT.Rows.Count)
                    {
                        ...
                        geuSum_dolg = geuSum_dolg + Decimal.Parse(DT.Rows[i]["dolg"].ToString());
                        ...
                        allSum_outsaldo = allSum_outsaldo + Decimal.Parse(DT.Rows[i]["sum_outsaldo"].ToString());
                        allSum_dolg = allSum_dolg + Decimal.Parse(DT.Rows[i]["dolg"].ToString());
                        ...
                        ExcelL.ExlWs.Cells[ExcelRow, 13] = DT.Rows[i]["dolg"].ToString();
                        ExcelRow++;
                        i++;
                    }
                    #endregion
                    //sw.WriteLine("тело создали");
                    ExcelL.ExlWs.Cells[rowGeu, 1] = oldGeu.Trim();
                    ExcelL.ExlWs.Cells[rowGeu, 2] = geuCount_ls;
                    ExcelL.ExlWs.Cells[rowGeu, 7] = geuSum_insaldo;
                    ExcelL.ExlWs.Cells[rowGeu, 8] = geuSum_charge;
                    ExcelL.ExlWs.Cells[rowGeu, 9] = geuSum_money;
                    ExcelL.ExlWs.Cells[rowGeu, 10] = geuSum_outsaldo;
                    ExcelL.ExlWs.Cells[rowGeu, 13] = geuSum_dolg;
                    excells1 = ExcelL.ExlWs.get_Range("A" + rowGeu.ToString(), "L" + rowGeu.ToString());
                    excells1.Font.Bold = true;
                    ...
                    ExcelL.ExlWs.Cells[7, 13] = allSum_dolg;
                    excells1 = ExcelL.ExlWs.get_Range("A7", "M7");
                    excells1.Font.Bold = true;
                    ...
                    excells1 = ExcelL.ExlWs.get_Range("A7", "M" + ExcelRow.ToString());
                    excells1.Borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous;
                    excells1.Font.Size = 6;
                    ...
                    #endregion
                    //sw.WriteLine("сохранили");
                }

                else
                {
                    //sw.WriteLine("DT = NULLLL!!!");
                    MonitorLog.WriteLog("DT = NULL!!!!", MonitorLog.typelog.Warn, true);
                    ret.result = false;
        ...
        public Returns GetSpravSoderg8(Prm prm, string nzpUser, ref string fileName)
        {
            ...
            string nameSupp = "";
            Dictionary<string, string> streets = new Dictionary<string, string>();
            //StreamWriter sw = new StreamWriter(@"C:\temp\GetSpravSoderg8.txt", false);
            streets.Add(" ГЕОРГИЯ МИТИРЕВА", " ГЕОРГИЯ МИТИРЕВА ПРОЕЗД");
            streets.Add(" САДОВЫЙ", " САДОВЫЙ ПРОЕЗД");
            streets.Add(" МАЛЬЦЕВА", " МАЛЬЦЕВА ПРОЕЗД");
            //создание dataTable
            ...
            try
            {
                ...
                if (dt != null)
                {
                    ...
                    try
                    {
                        ...
                        fileName = GetFileName(Global.Constants.ExcelDir,
                            prm.month_ + "_Дислокация_Отопление_" +
                             "_" + nzpUser) + ".xls";
                        ret = excelL.SaveFile(Global.Constants.ExcelDir + fileName, fileName, ref excelL.ExlWb, ref excelL.ExlApp);
                        excelL.ExlWb.Close(false, Type.Missing, Type.Missing);
                        excelL.ExlWb = null;
        ...
        public Returns GetSpravSoderg(Prm prm, string nzpUser, ref string fileName)
        {
            ...
            try
            {
                ...
                if (dt != null)
                {
                    ...
                    bool hasOdn = ((prm.nzp_serv == 6) || (prm.nzp_serv == 25) || (prm.nzp_serv == 14) || (prm.nzp_serv == 10) || (prm.nzp_serv == 210));

                    // Массив заголовков
                    var columnHeaderList = new List<string>();
                    columnHeaderList.Add("Начислено (тариф*кол-во), руб.");//0
                    columnHeaderList.Add("Постоянное начисление, руб.");
                    if (hasOdn)
                    {
                        columnHeaderList.Add("Начислено ОДН, руб.");
                    }
                    columnHeaderList.Add("Возврат за услуги, руб.");//3
                    columnHeaderList.Add("Начислено раз. красным, руб.");//4
                    columnHeaderList.Add("Начислено раз черным, руб.");//5
                    columnHeaderList.Add("Итого к оплате, руб.");//6
                    ...
                    while (i < dt.Rows.Count)
                    {
                        ...
                        if (oldAdres != adres)
                        {
                            ...
                            {
                                //sw.WriteLine("fieldList = " + fieldList[j]);
                                //sw.WriteLine("value = " + domMas[j]);
                        ...
                        for (int k = 0; k < fieldList.Count; k++)
                        {
                            try
                            {
                                //sw.WriteLine("fieldList2 = " + fieldList[k]);
                                //sw.WriteLine("value2 = " + domMas[k]);
                        ...
                        for (int k = 0; k < fieldList.Count; k++)
                        {
                            try
                            {
                                //sw.WriteLine("fieldList3 = " + fieldList[k]);
                                // sw.WriteLine("value3 = " + domMas[k]);
                                if ((k != 7 && hasOdn) || (k != 6 && !hasOdn))
                                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int j = 0; j < fieldList.Count - 3; j++)
                        {
                            //sw.WriteLine("fieldList4 = " + fieldList[j]);
                            //sw.WriteLine("value4 = " + domMas[j]);
                            if (Math.Abs(domMas[j]) > 0.001m)
                                excelL.ExlWs.Cells[excelRow, excelCol + j] = domMas[j];
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        excells1 = excelL.ExlWs.get_Range("C8", "M" + excelRow.ToString());
                        excells1.HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlRight;
                        excells1.NumberFormatLocal = "# ##0,00";
                    ...
                    try
                    {
                        ...
                        fileName = GetFileName(Global.Constants.ExcelDir, prm.month_ + "_Дислокация_" +
                             "_" + nzpUser) + ".xls";
                        ret = excelL.SaveFile(Global.Constants.ExcelDir + fileName, fileName, ref excelL.ExlWb, ref excelL.ExlApp);
                        if (InputOutput.useFtp)
                        {

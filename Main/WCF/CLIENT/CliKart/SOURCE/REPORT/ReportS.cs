...
namespace STCLINE.KP50.Client
{
    ...
    public class ReportS
    {
        ...
        public string SelectProc(Report Report, string reportFile, Int32 ye_, Int32 mo_, int nzp_user1, int CurT_nzp_kvar, string CurT_pref, int CurT_nzp_Kart, int CurT_nzp_user, bool CurT_Kart_Archive, List<_RolesVal> RolesVal, Email email, out Returns res)
        {
            //временно
            ...
            {
                return Fill_web_sost_fam(Report, ye_, mo_);
            }
        ...
        public string Fill_web_sost_fam(Report rep, int y_, int m_)
        {
            Returns ret = Utils.InitReturns();
            ...
            List<GilecFullInf> listgil = null;
            listgil = cli.CallFullInfGilList(finder, out ret);  //cli.Fill_web_sost_fam(rep, y_, m_, out ret, finder);
            ...
            rep.SetParameterValue("fam", "");
            rep.SetParameterValue("ima", "");
            rep.SetParameterValue("otch", "");

            #region Выборка данных
            //Returns ret = Utils.InitReturns();

            //Ls finder = new Ls();
            //finder.nzp_user = web_user.nzp_user;
            //finder.nzp_kvar = CurT_nzp_kvar;
            ////finder.num_ls = CurT_nzp_kvar;
            //finder.pref = CurT_pref;
                ...
                if (listgil.Count > 0)
                {
                    for (int i = 0; i < listgil.Count; i++)
                    {
                        table.Rows.Add(listgil[i].fam + " " +
                         listgil[i].ima + " " +
                         listgil[i].otch,
                         System.Convert.ToDateTime(listgil[i].dat_rog).ToShortDateString(),
                         listgil[i].rod);
                        if (listgil[i].nzp_kart == CurT_nzp_Kart)//(listgil[i].rod.ToUpper().Trim() == "НАНИМАТЕЛЬ")
                        {
                            rep.SetParameterValue("fam", listgil[i].fam);
                            rep.SetParameterValue("ima", listgil[i].ima);
                            rep.SetParameterValue("otch", listgil[i].otch);
                        }
            ...
            rep.SetParameterValue("dolg_oper", "");
            rep.SetParameterValue("fio_oper", "");
            rep.SetParameterValue("dolg_nach", "");
            rep.SetParameterValue("fio_nach", "");
        ...
        public string Fill_web_vip_dom(Report rep, int y_, int m_)
        {
            ...
            table.Columns.Add("cel", typeof(string));
            table.Columns.Add("rod", typeof(string));
            ...
            table.Columns.Add("jobpost", typeof(string));
            table.Columns.Add("dat_prib", typeof(string));
            table.Columns.Add("type_prop", typeof(string));
            table.Columns.Add("dat1", typeof(string));
            table.Columns.Add("dat_vip", typeof(string));
            table.Columns.Add("type_prop1", typeof(string));
            table.Columns.Add("m_ku", typeof(string));
            ...
            Ls finderLs = new Ls();
            finderLs.nzp_user = nzp_user;
            finderLs.nzp_kvar = CurT_nzp_kvar;
            finderLs.pref = CurT_pref;

            cli_Adres cliAdres = new cli_Adres(WCFParams.AdresWcfWeb.CurT_Server);
            List<Ls> listLs = cliAdres.LoadLs(finderLs, out ret);

            bool privat = false;

            foreach (var param in listLs[0].dopParams)
            {
                if (param.nzp_prm == 8)
                {
                    privat = (param.val_prm == "да" ? true : false);
                }
            }

            if (privat == false)
            {
                rep.SetParameterValue("privatiz", "Государственная");
            }
            else
            {
                rep.SetParameterValue("privatiz", "Приватизированная");
            }
            ...
            if (listgil != null & ret.result)
            {
                if (listgil.Count > 0)
                {
                    for (int i = 0; i < listgil.Count; i++)
                    {
                        ...
                        if (listgil[i].nzp_tkrt == "1")
                        {
                            if (listgil[i].tprp == "В")
                            {
                                dat1 = listgil[i].dat_oprp;
                            }
                        }
                        else
                        {
                            m_ku = listgil[i].landku + " " + listgil[i].statku + " " +
                            listgil[i].twnku + " " + listgil[i].rajonku + " " +
                            listgil[i].rem_ku;

                            if (listgil[i].tprp == "В")
                            {
                                type_prop1 = "временная";
                                if (listgil[i].dat_oprp.Trim() != "")
                                    dat1 = System.Convert.ToDateTime(listgil[i].dat_oprp).ToShortDateString();
                                else
                                    dat1 = "";
                            }
                        }
                        ...
                        if (listgil[i].vid_dat.Trim() != "")
                            vid_dat = System.Convert.ToDateTime(listgil[i].vid_dat).ToShortDateString();
                        else
                            vid_dat = "";
                        ...
                        table.Rows.Add(listgil[i].fam, listgil[i].ima, listgil[i].otch,
                         dat_rog,
                         listgil[i].landop, listgil[i].statop, listgil[i].twnop, listgil[i].rajonop,
                         listgil[i].rem_op, listgil[i].cel, listgil[i].rod, listgil[i].serij,
                         listgil[i].nomer,
                         vid_dat,
                         listgil[i].vid_mes,
                         listgil[i].job_name, listgil[i].job_post,
                         dat_prib,
                         listgil[i].type_prop, dat1,
                         dat_vip,
                         type_prop1, m_ku);
                    ...
                    if (NachInfo != null)
                    {
                        rep.SetParameterValue("fim_pasportist", (NachInfo[0].Trim() != "-" ? "/" + NachInfo[0].Trim() + "/" : ""));
                        rep.SetParameterValue("dolgnost_pasport", (NachInfo[1].Trim() != "-" ? NachInfo[1].Trim() : ""));
                        rep.SetParameterValue("dolgnost_nach", (NachInfo[2].Trim() != "-" ? NachInfo[2].Trim() : ""));
                        rep.SetParameterValue("fim_nachPus", (NachInfo[3].Trim() != "-" ? "/" + NachInfo[3].Trim() + "/" : ""));
                    }
                }
            }
        ...
        //public string Fill_web_saldo_rep5_10(Report rep, int y_, int m_)
        //{            
        //    #region Запонение таблицы для отчета
        //    DataSet FDataSet = new DataSet();
        ...
        //    finder.YM.month_ = m_;
        //    finder.YM.year_ = y_;
        //    finder.nzp_area = CurT_nzp_area;
        //    finder.nzp_dom = CurT_nzp_dom;

        //    List<SaldoRep> listRepData = null;
        //    List<SaldoRep> listRepDataGroup = new List<SaldoRep>();

        //    cli_Charge cli = new cli_Charge(WCFParams.AdresWcfWeb.CurT_Server);
        //    //новая процедура
        //    //listRepData = cli.FillRepServ(finder, out ret, 1);
        //    listRepData = cli.FillRep_5_10(finder, out ret, 1);
        ...
        //    listRepDataGroup = listRepData;

        //    #region Формирование списка данных на экран
        ...
        //    //        {
        //    //            b = false;
        //    //            for (int k = 0; k < listRepDataGroup.Count; k++)
        //    //                if (listRepDataGroup[k].nzp_serv == listRepData[i].nzp_serv)
        //    //                {
        //    //                    #region Накапливаем данные
        //    //                    listRepDataGroup[k].sum_insaldo = listRepDataGroup[k].sum_insaldo +
        //    //                        listRepData[i].sum_insaldo;

        //    //                    listRepDataGroup[k].sum_insaldo_k = listRepDataGroup[k].sum_insaldo_k +
        //    //                        listRepData[i].sum_insaldo_k;

        //    //                    listRepDataGroup[k].sum_insaldo_d = listRepDataGroup[k].sum_insaldo_d +
        //    //                        listRepData[i].sum_insaldo_d;

        //    //                    listRepDataGroup[k].reval = listRepDataGroup[k].reval +
        //    //                        listRepData[i].reval;

        //    //                    listRepDataGroup[k].real_charge = listRepDataGroup[k].real_charge +
        //    //                        listRepData[i].real_charge;

        //    //                    listRepDataGroup[k].sum_real = listRepDataGroup[k].sum_real +
        //    //                        listRepData[i].sum_real;

        //    //                    listRepDataGroup[k].sum_money = listRepDataGroup[k].sum_money +
        //    //                        listRepData[i].sum_money;

        //    //                    listRepDataGroup[k].sum_outsaldo_k = listRepDataGroup[k].sum_outsaldo_k +
        //    //                        listRepData[i].sum_outsaldo_k;

        //    //                    listRepDataGroup[k].sum_outsaldo_d = listRepDataGroup[k].sum_outsaldo_d +
        //    //                        listRepData[i].sum_outsaldo_d;

        //    //                    listRepDataGroup[k].sum_outsaldo = listRepDataGroup[k].sum_outsaldo +
        //    //                        listRepData[i].sum_outsaldo;
        //    //                    #endregion
        //    //                    b = true;
        //    //                    break;
        //    //                }
        ...
        //    //            {
        //    //                zap = new SaldoRep();
        //    //                zap.service = listRepData[i].service;
        //    //                zap.nzp_serv = listRepData[i].nzp_serv;
        //    //                zap.sum_insaldo = listRepData[i].sum_insaldo;
        //    //                zap.sum_insaldo_k = listRepData[i].sum_insaldo_k;
        //    //                zap.sum_insaldo_d = listRepData[i].sum_insaldo_d;
        //    //                zap.reval = listRepData[i].reval;
        //    //                zap.real_charge = listRepData[i].real_charge;
        //    //                zap.sum_real = listRepData[i].sum_real;
        //    //                zap.sum_money = listRepData[i].sum_money;
        //    //                zap.sum_outsaldo_k = listRepData[i].sum_outsaldo_k;
        //    //                zap.sum_outsaldo_d = listRepData[i].sum_outsaldo_d;
        //    //                zap.sum_outsaldo = listRepData[i].sum_outsaldo;
        //    //                listRepDataGroup.Add(zap);
        //    //            }
        ...
        //    for (int i = 0; i < listRepDataGroup.Count; i++)
        //    {
        //        table.Rows.Add(listRepDataGroup[i].service, listRepDataGroup[i].sum_insaldo_k,
        //            listRepDataGroup[i].sum_insaldo_d, listRepDataGroup[i].sum_insaldo,
        //            listRepDataGroup[i].reval, listRepDataGroup[i].real_charge,
        //            listRepDataGroup[i].sum_real, listRepDataGroup[i].sum_money,
        //            listRepDataGroup[i].sum_outsaldo_d, listRepDataGroup[i].sum_outsaldo_k,
        //            listRepDataGroup[i].sum_outsaldo);
        //    }
        ...
        public string Fill_web_listok_ubit(Report rep, int y_, int m_, DateTime date)
        {
            ...
            rep.SetParameterValue("pol", gender);

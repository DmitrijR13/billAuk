...
namespace STCLINE.KP50.Server
{
    public class srv_Debitor : srv_Base, I_Debitor
    {
        ...
        public Returns GetAllAgrementsReport(DateTime? dat_s, DateTime? dat_po, int user, int area)
        {
            ...
            ReportGen rp = new ReportGen();
            try
            {
                return ret = rp.GetListOfAgreemants(dat_s, dat_po, user, area, ReportType.Pdf);
            }
            catch (Exception ex)
            {
                ret.result = false;
                ret.text = "Ошибка вызова функции";
                if (Constants.Viewerror) ret.text += " : " + ex.Message;
                if (Constants.Debug) MonitorLog.WriteLog("Ошибка GetAllAgrementsReport() \n  " + ex.Message, MonitorLog.typelog.Error, 2, 100, true);
                return ret;
            }
        }

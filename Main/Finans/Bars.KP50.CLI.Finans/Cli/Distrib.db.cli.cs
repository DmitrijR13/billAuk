...
namespace STCLINE.KP50.DataBase
{
    ...
    /// </summary>
    public partial class DbDistribDomSuppClient : DataBaseHeadClient
    {
        private int nzpUser;

        public DbDistribDomSuppClient(int nzpUser)
        {
        ...
        public List<MoneyDistrib> GetDistribDom(MoneyDistrib finder, out Returns ret)
        {
            ...
            string select = "Select " + skip + " sum(sum_in) as sum_in, sum(sum_out) as sum_out" +
                          ...
                          ", sum(sum_reval) as sum_reval, sum(sum_charge) as sum_charge, sum(sum_send) as sum_send" +
                          ", min(nzp_dis) as nzp_dis, nzp_payer, max(payer) payer, nzp_payer_agent, max(agent) agent, nzp_payer_princip, max(princip) princip, nzp_payer_supp, max(supp) supp, nzp_serv, max(service) service, nzp_dom, max(adr) adr, nzp_bank, max(bank) bank, dat_oper";

            string from = " From " + tXXDistribDom;
            string groupBy = " Group by nzp_payer, nzp_payer_agent, nzp_payer_princip, nzp_payer_supp, nzp_serv, nzp_dom, nzp_bank, dat_oper";
            ...
            string sql = "select nzp_payer, nzp_payer_agent, nzp_payer_princip, nzp_payer_supp, nzp_serv, nzp_dom, nzp_bank, dat_oper " +
#if PG

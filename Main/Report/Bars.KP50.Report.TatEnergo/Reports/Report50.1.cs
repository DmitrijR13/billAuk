...
namespace Bars.KP50.Report.TatEnergo.Reports
{
    public class Report501 : BaseSqlReport
    {
        ...
        private void GetOneMonth(int i, string pref)
        {
           ...
           // string sumInsaldo = ((curMonth == MonthS) & (curYear == YearS)) ? "sum_insaldo" : "0";
            ...
            if (TempTableInWebCashe(tableCharge))
            {
                ...
                sql = " INSERT INTO t_temp_ensaldo (month_, year_, typek, nzp_kvar, " +
                      " sum_insaldo, sum_real, reval, real_charge, sum_money, sum_outsaldo) " +
                      " SELECT " + curMonth + ", " + curYear + ", " +
                      " typek, a.nzp_kvar, sum(sum_insaldo), " +
                      " sum(sum_real), sum(reval), sum(real_charge), sum(sum_money), " +
                      " sum(sum_outsaldo) " +
                      " FROM  " + tableCharge + " a, " + baseData + " kvar k, " + baseData + "dom d " +
                      " WHERE a.nzp_kvar = k.nzp_kvar AND d.nzp_dom = k.nzp_dom AND dat_charge IS NULL AND nzp_serv>1 " +
                      GetWhereAdr("k.") + GetWhereServ() + GetWhereSupp() + GetWhereRajon() +
                      " GROUP BY 1,2,3,4";
                ...
                sql =
                    ...
                    "       sum(CASE WHEN sum_insaldo > 36*sum_real THEN sum_insaldo-sum_real*36 ELSE 0 END) AS sum_in36, " +
                    ...
                    "       sum(CASE WHEN typek=1 THEN " + DBManager.sNvlWord + "(sum_money,0) ELSE 0 END) AS sum_money_n, " +
                    "       sum(CASE WHEN typek>1 THEN " + DBManager.sNvlWord + "(sum_money,0) ELSE 0 END) AS sum_money_a, " +
                    "       sum(CASE WHEN typek>=1 THEN " + DBManager.sNvlWord + "(sum_money,0) ELSE 0 END) AS sum_money, " +
                    "       sum(CASE WHEN typek=1 THEN sum_outsaldo ELSE 0 END) AS sum_outsaldo_n, " +
                    "       sum(CASE WHEN typek=1 " +
                    "           AND sum_outsaldo-sum_real-reval-real_charge > 36*sum_real" +
                    "           THEN sum_outsaldo-sum_real-reval-real_charge-36*sum_real  ELSE 0 END) AS sum_out36, " +
        ...
        {
            if (TempTableInWebCashe("t_temp_ensaldo"))
                ExecSQL("drop table t_temp_ensaldo"); 
            if (TempTableInWebCashe("t_ensaldo")) 
        ...
        protected override void PrepareReport(FastReport.Report report)
        {
            ...
            string headers = "Сальдовая ведомость за " ;
            ...
            string headerParam = !string.IsNullOrEmpty(RajonHeader) ? "Район: " + RajonHeader + "\n" : string.Empty;
        ...
        /// </summary>
        /// <returns></returns>
        private string GetWhereAdr(string tableName)
        {
            ...
            {
                whereAdr = " AND " + tableName + "nzp_area in (" + whereAdr + ")";


                if (String.IsNullOrEmpty(AreaHeader))
                {
                    string sql = " SELECT area from " +
                                 ReportParams.Pref + DBManager.sDataAliasRest + "s_area " + tableName.TrimEnd('.') +
        ...
        private string GetWhereRajon()
        {
            ...
            if (String.IsNullOrEmpty(whereRaj)) whereTown = ReportParams.GetRolesCondition(Constants.role_sql_town);
            whereTown = whereTown.TrimEnd(',');

            if (!String.IsNullOrEmpty(whereRaj))
            {
                whereRaj = " AND nzp_raj in (" + whereRaj + ")";

                //Поставщики
                if (String.IsNullOrEmpty(RajonHeader))
                {
                    string sql = " SELECT rajon from " + ReportParams.Pref + DBManager.sDataAliasRest + "s_rajon " +
                                 " WHERE nzp_raj > 0 " + whereRaj;
            ...
            else if (!string.IsNullOrEmpty(RajonHeader))
                whereRaj = " AND nzp_raj IN (SELECT nzp_raj FROM " + ReportParams.Pref + DBManager.sDataAliasRest + "s_rajon WHERE nzp_town IN (" + whereTown + ") ) ";
            return whereRaj;
        }
        ...
        protected override void PrepareParams()
        {
            ...
            {
                if (Convert.ToDateTime(ReportParams.User.date_begin) > new DateTime(YearS, MonthS, 01))
                {
                    MonthS = Convert.ToDateTime(ReportParams.User.date_begin).Month;
                    YearS = Convert.ToDateTime(ReportParams.User.date_begin).Year;
                }

...
{
  public  class Report1205020 : BaseSqlReport
    {
        public override string Name
        {
            get { return "12.5.20 - Сальдовая ведомость по лицевым счетам"; }
        }
        ...
        {
            get { return Resources.Report_12_5_20; }
        }
        ...
        public override DataSet GetData()
        {
            ...
            while (reader.Read())
            {
                ...
                for (int i = YearS*12 + MonthS; i < YearPo*12 + MonthPo + 1; i++)
                {
                    ...
                    {
                        sql =
                            " INSERT INTO t_report_12_5_20 (nzp_dom, nzp_kvar, num_ls, sum_insaldo_k, sum_insaldo_d, sum_insaldo, sum_real, reval, " +
                            " money_to, money_from, money_del, sum_outsaldo_k, sum_outsaldo_d, sum_outsaldo ) " +
                            " SELECT k.nzp_dom, k.nzp_kvar, k.num_ls,  " +
                        ...
                        ExecSQL(DBManager.sUpdStat + " t_report_12_5_20");

                        sql = " UPDATE t_report_12_5_20 " +
                              " SET real_charge = ( SELECT SUM(sum_rcl)" +
                              " FROM " + perekidka + " p " +
                              " WHERE p.type_rcl not in (100,20) " +
                              " AND p.nzp_kvar = t_report_12_5_20.nzp_kvar " + whereServ + GetWhereSupp("nzp_supp") +
                              " AND p.month_ = " + mo + ") ";
                        ExecSQL(sql);
                        ...
                        sql = " INSERT INTO t_report_12_5_20(num_ls, money_from, money_supp)" +
                              " SELECT num_ls , -SUM(sum_prih) AS money_from , SUM(sum_prih) AS money_supp " +
                              " FROM " + fromSupplier + " a " +
                              " WHERE a.kod_sum in (49, 50) " + whereServ + GetWhereSupp("a.nzp_supp") +
                        ...
                        sql =
                            " INSERT INTO t_all_12_5_20 ( nzp_dom, nzp_kvar, num_ls, name_serv, sum_insaldo_k, sum_insaldo_d, sum_insaldo, sum_real, " +
                            " reval, real_insaldo, real_charge, money_to, money_from, money_del,money_supp, sum_outsaldo_k, sum_outsaldo_d, sum_outsaldo)" +
                            " SELECT nzp_dom, nzp_kvar, num_ls, MAX(name_serv), SUM(sum_insaldo_k), SUM(sum_insaldo_d), SUM(sum_insaldo), SUM(sum_real), SUM(reval), SUM(real_insaldo), " +
                            " SUM(real_charge), SUM(money_to), SUM(money_from), SUM(money_del), SUM(money_supp), SUM(sum_outsaldo_k), SUM(sum_outsaldo_d), SUM(sum_outsaldo) " +
                            " FROM t_report_12_5_20 " +
                            " GROUP BY 1,2,3 ";
                        ExecSQL(sql);
                    }
                    ExecSQL(" DELETE FROM t_report_12_5_20 ");
                }
                if (_DivideServs == 1)
                {
                    sql = " UPDATE t_all_12_5_20 " +
                          " SET name_serv = ( SELECT 'Дома с услугой '|| Trim(val_prm) " +
                          " FROM " + pref + DBManager.sDataAliasRest + "prm_2 p " +
                          " WHERE p.nzp_prm=3000014 " +
                          " AND p.nzp = t_all_12_5_20.nzp_dom " +
                          " AND is_actual <> 100 )";
                    ExecSQL(sql);

                    sql = " UPDATE t_all_12_5_20 " +
                          " SET name_serv = 'Дома с услугой Теплоноситель' " +
                          " WHERE name_serv IS NULL ";
                    ExecSQL(sql);
                }
            }
            ...
            sql =
                " SELECT Trim(name_serv)as name_serv, a.num_ls, " + adres + " AS ulica, " +
        ...
        private bool GetSelectedKvars()
        {
            if (ReportParams.CurrentReportKind == ReportKind.ListLC)
            {
                using (IDbConnection connWeb = DBManager.GetConnection(Constants.cons_Webdata))
                {
                    ...
                    {
                        string sql = " insert into selected_kvars_5_20 (num_ls, nzp_kvar, nzp_dom, nzp_area ) " +
                                     " select num_ls, nzp_kvar, nzp_dom, nzp_area from " + tSpls;
                        ExecSQL(sql);
                        ExecSQL("create index ix_tmpsk_ls_01 on selected_kvars_5_20(nzp_kvar) ");
                        ExecSQL("create index ix_tmpsk_ls_02 on selected_kvars_5_20(nzp_dom) ");
        ...
        protected override void PrepareReport(FastReport.Report report)
        {
            ...
            report.SetParameterValue("_DivideServs", _DivideServs);
        ...
        protected override void CreateTempTable()
        {
            ...
            ExecSQL(" CREATE INDEX ix_t_report_12_5_20 on t_report_12_5_20(nzp_kvar) ");

            sql = " CREATE TEMP TABLE t_all_12_5_20( " +
                  " nzp_dom INTEGER DEFAULT 0, " +
                  " nzp_kvar INTEGER DEFAULT 0, " +
                  " num_ls INTEGER DEFAULT 0, " +
                  " name_serv CHAR(50) DEFAULT ' ', " +
                  ...
                  " sum_outsaldo " + DBManager.sDecimalType + "(14,2)) " + DBManager.sUnlogTempTable; 
            ExecSQL(sql);
            ExecSQL(" CREATE INDEX ix_t_all_12_5_20_1 on t_all_12_5_20(nzp_kvar) ");
            ExecSQL(" CREATE INDEX ix_t_all_12_5_20_2 on t_all_12_5_20(nzp_dom) ");
        ...
        {
            ExecSQL(" DROP TABLE t_report_12_5_20 ", true);
            ExecSQL(" DROP TABLE t_all_12_5_20 ", true);
            if (ReportParams.CurrentReportKind == ReportKind.ListLC)

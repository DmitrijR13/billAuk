...
namespace Bars.KP50.Report.Samara.Reports
{
    ...
    class ReportPackMoreTotal : BaseSqlReport
    {
        /// <summary>Наименование отчета, которое видет пользователь</summary>
        public override string Name
        {
            get { return "Распределение платежей детализация. Итого"; }
        }
        ...
        {
            get { return Resources.ReportPackMoreTotal; }
        }
        ...
        public override DataSet GetData()
        {
            ...
            Int32 days = DateTime.DaysInMonth(YearPo, MonthPo);

            for (int i = MonthS; i <= MonthPo; i++)
            {
                ...
                sql = @"INSERT INTO t_svod_pack
                        SELECT '01." + month + "." + (year).ToString("00") + @"' as date, s.service, sup.name_supp, c.sum_insaldo, c.rsum_tarif, c.reval - c.sum_nedop + coalesce(p.sum_rcl, 0) as reval,
            ...
            var ds = new DataSet();
            var dt2 = ExecSQLToTable("SELECT address, date, service, name_supp, sum(sum_insaldo) as sum_insaldo, sum(rsum_tarif) as rsum_tarif, sum(reval) as reval, sum(sum_charge) as sum_charge, " +
                                     "sum(sum_prih) as sum_prih, sum(sum_outsaldo) as sum_outsaldo FROM t_svod_pack group by 1,2,3,4 order by 1,2,3,4");
        ...
        {
            ExecSQL("create temp table t_svod_pack (date CHARACTER(10), service CHARACTER(100)," +

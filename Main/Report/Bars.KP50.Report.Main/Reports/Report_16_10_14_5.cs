...
namespace Bars.KP50.Report.Main.Reports
{
    class Report1610145 : BaseSqlReport
    {
            ...
            while (reader.Read())
            {
                ...
                sql = " SELECT area," +
                      ...
                      " FROM t_sald_obor_vedom_all a, " +
                           " OUTER " + ReportParams.Pref + DBManager.sKernelAliasRest + "services c," +
                           " OUTER " + ReportParams.Pref + DBManager.sDataAliasRest + "s_area sa " +
        ...
        private string GetWhereSupp()
        {
            ...
            if (Suppliers != null)
            {
                whereSupp = Areas.Aggregate(whereSupp, (current, nzpArea) => current + (nzpArea + ","));
            }
            else
            {
                whereSupp = ReportParams.GetRolesCondition(Constants.role_sql_area);
            }
            ...
            if (!String.IsNullOrEmpty(whereSupp))
            {
                ...
                {
                    SupplierHeader += dr["area"].ToString().Trim() + ",";
                }

...
{
    class SpisDomGil : BaseSqlReport
    {
        ...
        public override DataSet GetData()
        {
            ...
            if (Status == 1)
            {
                sql.Append(" insert into sel_kvar (nzp_kvar, ikvar, nzp_dom, idom, nkor, nzp_ul, ulica, nzp_raj, rajon, nzp_town, town) " +
                           " select " + DBManager.sUniqueWord + " k.nzp_kvar, k.ikvar, d.nzp_dom, d.idom, d.nkor, u.nzp_ul, u.ulica, r.nzp_raj, r.rajon, t.nzp_town, t.town " +
                           " from " + ReportParams.Pref + DBManager.sDataAliasRest + "kvar k, " +
                             ReportParams.Pref + DBManager.sDataAliasRest + "dom d, " +
                             ReportParams.Pref + DBManager.sDataAliasRest + "s_ulica u, " +
            ...
            while (reader.Read())
            {
                ...
                sql.Append(" insert into t_gil(town, rajon, ulica, idom, nkor, pl_dom, kolls, kolkvar, kolgil) " + 
                    " select town, rajon, ulica, idom, nkor, sum(replace(val_prm,',','.')" + DBManager.sConvToInt + ") as pl_dom, " +
                    " count(sk.nzp_kvar) as kolls, count(distinct ikvar) as kolkvar, sum(round(gil)) as kolgil " +
                    " from sel_kvar sk " +
                    " left outer join " + pref + DBManager.sDataAliasRest + "prm_1 p1 on (sk.nzp_kvar = p1.nzp) " +
                    " left outer join " + pref + "_charge_" + (DatS.Year - 2000).ToString("00") +
                      DBManager.tableDelimiter + "calc_gku_" + DatS.Month.ToString("00") + " cg on (sk.nzp_kvar = cg.nzp_kvar) " +
                    " where p1.nzp_prm=4 and p1.is_actual<>100 " +

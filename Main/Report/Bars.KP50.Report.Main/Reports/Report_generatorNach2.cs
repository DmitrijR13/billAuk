...
namespace Bars.KP50.Report.Main.Reports
{
    class GenNach2 : BaseSqlReport
    {
        ...
        public override DataSet GetData()
        {
                    ...
                    TempTableInWebCashe(chargeTable) && TempTableInWebCashe(prmTable))
                {
                    ...
                    if (Params.Contains(11) || Params.Contains(12) || Params.Contains(13) || Params.Contains(14) || Params.Contains(15))
                    {
                        ...
                        sql.Append("  insert into t_s (nzp_kvar, ob_s, otop_s, gil_s, rooms, floor) " +
                                   "  select nzp as nzp_kvar, " +
                                   "  case when nzp_prm = 4 AND " + DBManager.sNvlWord + "(trim(val_prm), '') <> ''" +
                                   "  then p.val_prm" + DBManager.sConvToNum + " end as ob_s, " +
                                   "  case when nzp_prm = 133 AND " + DBManager.sNvlWord + "(trim(val_prm), '') <> ''" +
                                   "  then p.val_prm" + DBManager.sConvToNum + " end as otop_s, " +
                                   "  case when nzp_prm = 6 AND " + DBManager.sNvlWord + "(trim(val_prm), '') <> ''" +
                                   "  then p.val_prm" + DBManager.sConvToNum + " end as gil_s, " +
                                   "  case when nzp_prm = 107 AND " + DBManager.sNvlWord + "(trim(val_prm), '') <> ''" +
                                   "  then p.val_prm" + DBManager.sConvToInt + " end as rooms, " +
                                   "  case when nzp_prm = 2 AND " + DBManager.sNvlWord + "(trim(val_prm), '') <> ''" +
                                   "  then p.val_prm" + DBManager.sConvToInt + " end as floor " +
                                   "  from " + prmTable + " p " +
            ...
            try
            {
                dt = ExecSQLToTable(" select area, geu, uch, rajon, ulica, idom, ndom, nkor, ikvar, nkvar, num_ls, fio, " +
                    " propis_count, gil_prib, gil_ub, ob_s, otop_s, gil_s, rooms, floor, pkod, name_supp, service, " +
                                    " Round((CASE WHEN sum_tarif = 0 THEN 0 " +
                                    " ELSE (sum_insaldo - sum_money)/sum_tarif END),0) AS srok_dolg," +
                    " sum_insaldo, tarif, rsum_tarif, sum_nedop, sum_tarif, reval, real_charge, sum_charge, sum_money, sum_outsaldo " +
                    " from t_svod_all ", 1000);
            ...
            {
                dt = ExecSQLToTable(DBManager.SetLimitOffset(" select area, geu, uch, rajon, ulica, idom, ndom, nkor,ikvar,  nkvar, num_ls, fio, " +
                    " propis_count, gil_prib, gil_ub, ob_s, otop_s, gil_s, rooms, floor, pkod, name_supp, service," +
                                    " Round((CASE WHEN sum_tarif = 0 THEN 0 " +
                                    " ELSE (sum_insaldo - sum_money)/sum_tarif END),0) AS srok_dolg," +
                    " sum_insaldo, tarif, rsum_tarif, sum_nedop, sum_tarif, reval, real_charge, sum_charge, sum_money, sum_outsaldo " +
                    " from t_svod_all ", 100000, 0), 1000);
                var dv = new DataView(dt) { Sort = order };
                dt = dv.ToTable();
                RowCount = true;
            }
        ...
        private string FillSvodTable()
        {
            ...
            if (Params.Contains(4)) { sql.Append(" nzp_raj, nzp_ul, "); }
            if (Params.Contains(5)) { sql.Append(" nzp_dom, "); }
            //для параметров тоже нужно это поле
            if (Params.Contains(6) || Params.Contains(9) || Params.Contains(10) || Params.Contains(11) ||
                Params.Contains(12) || Params.Contains(13) || Params.Contains(14) || Params.Contains(15) || 
                Params.Contains(16)) { sql.Append(" nzp_kvar, "); }
            if (Params.Contains(7)) { sql.Append(" num_ls, "); }
            if (Params.Contains(8)) { sql.Append(" fio, "); }
            if (Params.Contains(17)) { sql.Append(" pkod, "); }
            if (Params.Contains(18)) { sql.Append(" nzp_supp, "); }
            ...
            if (Params.Contains(6) || Params.Contains(9) || Params.Contains(10) || Params.Contains(11) ||
                Params.Contains(12) || Params.Contains(13) || Params.Contains(14) || Params.Contains(15) ||
                Params.Contains(16)) { sql.Append(" m.nzp_kvar, "); grouper.Append(" nzp_kvar, ");  }
            if (Params.Contains(6)) { order.Append(" ikvar, nkvar, "); }
            ...
            if (Params.Contains(22)) { sql.Append(" tarif, "); grouper.Append(" tarif, "); order.Append(" tarif, "); }
            ...
            sql.Append(!String.IsNullOrEmpty(grouper.ToString()) ? " group by " + grouper : "");
            ExecSQL(sql.ToString());
            ...
            if (Params.Contains(6))
            {
                ...
                ExecSQL(sql.ToString());
                sql.Remove(0, sql.Length);
                sql.Append(" update t_svod_all set ikvar = (" +
        ...
        private void CreateTSvod()
        {
            ...
            sql.Append(" nzp_raj integer, nzp_ul integer, rajon char(100), ulica char(100), "); 
            ...
            if (Params.Contains(8)) { sql.Append(" fio character(100), "); }

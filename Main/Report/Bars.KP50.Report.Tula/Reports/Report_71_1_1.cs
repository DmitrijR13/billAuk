...
namespace Bars.KP50.Report.Tula.Reports
{
    ...
    public class Report7111 : BaseSqlReport
    {
        public override string Name
        {
            get { return "71.1.1 Сводный отчет по начислениям"; }
        }
        ...
        {
            get { return Resources.Report_71_1_1; }
        }
        ...
        public override DataSet GetData()
        {
            ...
            while (reader.Read())
            {
                ...
                for (int i = DatS.Year*12 + DatS.Month; i < DatPo.Year*12 + DatPo.Month + 1; i++)
                {
                    ...
                    sql = " INSERT INTO t_nach (nzp_serv, nzp_supp, sum_nach )" +
                          " SELECT  a.nzp_serv, a.nzp_supp, sum(" + vidNach + ")  " +
                          " FROM " + chargeXx + " a, " +
                    ...
                    if (TypeNacl == 3)
                    {
                        var perekidka = pref + "_charge_" + (year - 2000).ToString("00") + DBManager.tableDelimiter +
                                        "perekidka";
                        sql = " INSERT INTO t_nach (nzp_serv, nzp_supp, sum_nach )" +
                              " SELECT  a.nzp_serv, a.nzp_supp, -sum(sum_rcl)  " +
                              " FROM " + perekidka + " a, " +
                              (listLc ? " selected_kvars k " : pref + DBManager.sDataAliasRest + "kvar k ") +
            ...
            sql = " SELECT sa.payer as agent, sp.payer as principal, s.service, " +

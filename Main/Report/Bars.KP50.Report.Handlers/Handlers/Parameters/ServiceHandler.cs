namespace Bars.KP50.Report.Handlers.Parameters
{
    ...
    public class ServiceHandler : JsonRequestHandler
    {
        public override void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                Returns ret;
           //     var servList = cliSprav.ServiceLoad(new Finder { webLogin = context.User.Identity.Name, pref = pref }, out ret);
                ...
                var servList = cliSprav.ServiceLoad(finder, out ret);
                if (servList != null)
                {
                    result.AddRange(servList.Select(x => new { Id = x.nzp_serv, Name = x.service }).ToArray().OrderBy(x => x.Name));
                }

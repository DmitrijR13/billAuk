namespace Bars.KP50.Report.Handlers.Parameters
{
    ...
    public class SupplierRealHandler : JsonRequestHandler
    {
        public override void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
               ...
               // var servList = cliSprav.SupplierLoad(new Finder { webLogin = context.User.Identity.Name, pref = pref }, enTypeOfSupp.None, out ret);
                ...
                var servList = cliSprav.PayerBankLoad(finder, enSrvOper.Supplier, out ret);
                if (servList != null)
                {
                    result.AddRange(servList.Select(x => new { Id = x.nzp_payer, Name = x.payer }).ToArray());
                }
            ...
            {
                MonitorLog.WriteLog("Ошибка загрузки принципалов : " + ex.Message, MonitorLog.typelog.Error, true);
            }
        }

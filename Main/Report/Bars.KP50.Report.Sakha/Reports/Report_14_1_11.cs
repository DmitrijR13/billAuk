...
{
    public class Report140111 : BaseSqlReport
    {
      public override string Name
        {
            get { return "14.1.11 Свод по оплате за ЖКУ до закрытия опер.дня"; }
        }

        public override string Description
        {
            get { return "Свод по оплате за ЖКУ до закрытия опер.дня"; }
        }
        ...
        {
            get { return Resources.Report_14_1_11; }
        }
        ...
        public override DataSet GetData()
        {
            ...
            string whereFrmPl = GetFormingPlace();
            ...
            foreach (string curPref in PrefList)
            {
                for (var i = DatS.Year * 12 + DatS.Month; i <= DatPo.Year * 12 + DatPo.Month; i++)
                {
                    ...
                    sql = " INSERT INTO t_14_11_sums (nzp_area, sum_money, sum_peni) " +
                          " SELECT nzp_area, sum (case when nzp_serv<>500 then sum_prih else 0 end), sum (case when nzp_serv=500 then sum_prih else 0 end)  " +
                          " FROM " + pack_ls + " pl, " +
                          pack + " p, " +
                          supplier + " s, " +
        ...
        protected override void PrepareReport(FastReport.Report report)
        {
            ...
            headerParam += !string.IsNullOrEmpty(PlaceHeader) ? "Кассы: " + PlaceHeader + "\n" : "";
            headerParam += "Виды денег: наличные " ;
        ...
        {
            const string sql = " create temp table t_14_11_sums (     " +
        ...
        {
            ExecSQL(" drop table t_14_11_sums ", false);
        }
        ...
        /// <summary>
        /// Получить условия органичения по местам формирования
        /// </summary>
        /// <returns></returns>
        private string GetFormingPlace()
        {
            ...
            string Res = (FormingPlace != 0) ? " and p.nzp_bank in ( select sb.nzp_bank from " + ReportParams.Pref + DBManager.sKernelAliasRest + "s_bank sb where  sb.nzp_payer in(" + FormingPlace + "))" : String.Empty;
            return Res;
        }

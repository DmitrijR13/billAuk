...
namespace STCLINE.KP50.DataBase
{
    public partial class Debitor : DataBaseHead
    {
        ...
        public Returns CheckDealsBeforeGroupOperation(Deal finder, int nzp_oper)
        {
            ...
            ret = ExecSQL(conn, "SET search_path TO public;", true);
            if (!TempTableInWebCashe(conn, "t" + finder.nzp_user + "_deal"))
            {
                conn.Close();
                ret.result = false;
        ...
        public int AddGroupOperation(Deal finder, int nzp_oper, ReportType type, out Returns ret)
        {
            ...
            sql.Append(" (SELECT DISTINCT nzp_kvar FROM public.t" + finder.nzp_user + "_deal where mark = 1) ");
            #endregion
        ...
        public List<Supplier> GetListSupplier(Deal finder, out Returns ret)
        {
            ...
            DateTime calcdate = new DateTime(rm.year_, rm.month_, 1);
            string curr_charge = pref + "_charge_" + (rm.year_ % 100).ToString("00") + ".charge_" + rm.month_.ToString("00");
            string prev_charge = pref + "_charge_" + (rm.month_ > 1
                ? rm.year_ % 100
                : (rm.year_ - 1) % 100
                ).ToString("00") + ".charge_" +
                (rm.month_ > 1
            ...
            sql.Append(" FROM " + pref + "_charge_" + (rm.year_ % 100).ToString("00") + ".perekidka ");
            ...
            sql.Append(" and c1.nzp_supp>0 and c2.nzp_supp>0 ");
            #endregion
        ...
        public List<Service> GetListServices(Deal finder, int nzp_supp, out Returns ret)
        {
            ...
            #region SQL select suppliers
            RecordMonth rm = Points.GetCalcMonth(new CalcMonthParams(pref));
            DateTime calcdate = new DateTime(rm.year_, Points.CalcMonth.month_, 1);
            string curr_charge = pref + "_charge_" + (rm.year_ % 100).ToString("00") + ".charge_" + rm.month_.ToString("00");
            string prev_charge = pref + "_charge_" + (rm.month_ > 1
                ? rm.year_ % 100
                : (rm.year_ - 1) % 100
           ...
           // StringBuilder sql = new StringBuilder();
            string sql = " SELECT  c1.nzp_serv,  s.service_name,  c1.sum_insaldo - c1.sum_money + (  CASE  WHEN c1.reval < 0 THEN  c1.reval  ELSE  0  END  ) " +
                         " + (  CASE  WHEN c1.real_charge < 0 THEN  c1.real_charge  ELSE  0  END  ) - COALESCE ( c2.sum_money - (  CASE  WHEN c2.reval < 0 THEN " +
                         " c2.reval  ELSE  0  END  ) - (  CASE  WHEN c2.real_charge < 0 THEN  c2.real_charge  ELSE  0  END  ),  0  ) as dolg  " +
                         " from " + prev_charge + " c1 left outer JOIN  " + curr_charge + " c2 ON c1.nzp_kvar = c2.nzp_kvar  AND c1.nzp_serv = c2.nzp_serv  AND c1.nzp_supp = c2.nzp_supp and  c2.dat_charge is null, " +
                         pref + sKernelAliasRest + "services s  where c1.nzp_kvar = " + nzp_kvar + " AND c1.nzp_serv = s.nzp_serv  AND c1.nzp_supp = "+nzp_supp+" AND  c1.sum_insaldo - c1.sum_money " +
                         " + (  CASE  WHEN c1.reval < 0 THEN  c1.reval  ELSE  0  END  ) + (  CASE  WHEN c1.real_charge < 0 THEN  c1.real_charge  ELSE  0  END  ) " +
                         "  - COALESCE (  c2.sum_money - (  CASE  WHEN c2.reval < 0 THEN  c2.reval  ELSE  0  END  ) - (  CASE  WHEN c2.real_charge < 0 " +
                         " THEN  c2.real_charge  ELSE  0  END  ),  0  )  > 0  and c1.dat_charge is null  and c1.nzp_serv>1 and c2.nzp_serv>1 ";
            ...
            var supps = ClassDBUtils.OpenSQL(sql, conn);
            if (supps.resultCode != 0)
            {
                ret.result = false;
            ...
            try
            {
                List<Service> res = supps.resultData.AsEnumerable().Select(x => new Service()
                {
                    ...
                    service_name = Convert.ToString(x["service_name"]).Trim()
                    + " (" + Convert.ToDecimal(x["dolg"]).ToString("G") + " руб.)"
                }).ToList();
                return res;
            }
            catch (Exception ex)
        ...
        public Returns AddPerekidka(Deal finder, decimal money)
        {
            ...
            ret = ExecSQL(conn, trans, "INSERT INTO " + pref + "_charge_" + (Points.CalcMonth.year_  % 100).ToString("00")+ ".perekidka " +
                ...
                nzp_kvar + "," + nzp_kvar + "," + finder.nzp_serv + "," + finder.nzp_supp + ",255,'" + calcdate.ToString("yyyy-MM-dd") + "',0,0,'-" + money.ToString("G") + "'," + rm.month_ + ",'Списание долга'," + finder.nzp_user + ");", true);
            ...
            #region Обновить сумму долга в пользовательской таблице txx_debt

            string txx_debt = "t" + finder.nzp_user + "_debt";
            if (TempTableInWebCashe(conn, txx_debt))
            {
                string updSql = "update " + txx_debt + " set debt_money=debt_money-" + money.ToString("G") + " where nzp_kvar=" + nzp_kvar;
                ret = ExecSQL(conn, updSql, true);
                if (!ret.result)
                {
                    trans.Rollback();
                    conn.Close();
            ...
            DbAdmin.InsertSysEvent(new SysEvents()
            {
                ...
                nzp_dict = 6597,// означает добавление перекидки
                nzp_obj = nzp_kvar,
                note =
                    "Перекидка на сумму " + money.ToString("G") + " была " +
                     "добавлена "
            ...
            dbCalc.CalcChargeXXUchetOplatForLs(conn, null,
                new Charge()
                {
                    nzp_kvar = nzp_kvar,
                    pref = pref,

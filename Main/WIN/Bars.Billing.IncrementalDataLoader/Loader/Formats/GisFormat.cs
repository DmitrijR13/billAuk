...
{
    [Assembly(FormatName = "ГИС", Version = "1.0", RegistrationName = "GIS")]
    public class GisFormat : Format
    {
        ...
        protected override string Run()
        {
            ...
            try
            {
                ...
                parentDir = string.IsNullOrEmpty(AbsolutePath)
                    ? (new FileInfo(Assembly.GetEntryAssembly().Location)).Directory.FullName
                    : AbsolutePath;
                newPath =
                    Directory.CreateDirectory(string.Format("{0}\\LoadFiles\\{1}\\{2}\\{3}\\{4}\\" + nzp_load, parentDir,
                        DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, FileName.Split('.').First()));
                scriptsPath =
                    Directory.CreateDirectory(
                        string.Format("{0}\\iso_folder.exp\\pg_" + nzp_load + "\\convert_" + nzp_load, parentDir));
                ...
                Archive.GetInstance(newPath.FullName + "\\" + FileName)
                    .Decompress(newPath.FullName + "\\" + FileName, newPath.FullName);
                ...
                try
                {
                    ...
                    headerfileLines = File.ReadAllLines(newPath.FullName + "\\_main.txt", Encoding.GetEncoding(1251));
                    {
                        ...
                        Connection.Execute(
                            ...
                            nzp_load, header);
                        DateCharge = Convert.ToDateTime(header.chargeDate);
                        Schema =
                            Connection.ExecuteScalar<string>(
                                "select pref from gkh_pref where upper(id_case) = upper('" + schema_code +
                                "')", null, null, null, null).Trim() + "_" + schema_pref;
                        if (string.IsNullOrEmpty(Schema))
                            throw new Exception("Не определена схема");
                        if (!new Db(ConnString).CheckGkh(schema_code, schema_pref,
                            Convert.ToDateTime(header.chargeDate)))
                        {
                            throw new Exception(
                                "Не возможно загрузить данные, т.к. в базе данных есть не все связи. Порядок загрузки следующий: kernel,data,fin или charge");
                        }
                ...
                for (var i = 1; i < headerfileLines.Count(); i++)
                {
                    ...
                    else if (!split[1].Trim().Contains("recalc") ||
                             split[1].Trim().Contains("recalc") && split[1].Trim().Contains(DateCharge.Year.ToString()))
                    {
                ...
                if (Schema.Contains("data"))
                {
                    ...
                    var info1 = new ProcessStartInfo("cmd.exe",
                        string.Format(
                            "/C call \"" + AbsolutePath +
                            "\\gkh_triggers_data_off.bat\" \"{0}\" \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{6}\"",
                ...
                {
                    {
                        ...
                        string.Format(
                            "/C call \"" + AbsolutePath +
                            "\\gkh_del.bat\" \"{0}\" \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{8}\" \"{6}\" \"{7}\" 2>\"" +
                            logName + "\"", Schema, PsqlPath, scriptsPath.FullName, Server, Port, Database, Password,
                            nzp_load, User)
                    },
                    {
                        updDict,
                        string.Format(
                            "/C call \"" + AbsolutePath +
                            "\\gkh_upd.bat\" \"{0}\" \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{9}\" \"{6}\" \"{7}\" \"{8}\" 2>\"" +
                            logName + "\"", Schema, PsqlPath, scriptsPath.FullName, Server, Port, Database, Password,
                            nzp_load, DateCharge.ToShortDateString(), User)
                    },
                    {
                        packLsDict,
                        string.Format(
                            "/C call \"" + AbsolutePath +
                            "\\gkh_pack_ls.bat\" \"{0}\" \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{8}\" \"{6}\" \"{7}\" 2>\"" +
                            logName + "\"", Schema, PsqlPath, scriptsPath.FullName, Server, Port, Database, Password,
                            nzp_load, User)
                    },
                    {
                        ...
                        string.Format(
                            "/C call \"" + AbsolutePath +
                            "\\gkh_must_calc.bat\" \"{0}\"  \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{9}\" \"{6}\" \"{7}\" \"{8}\" 2>\"" +
                            logName + "\"", Schema, PsqlPath, scriptsPath.FullName, Server, Port, Database, Password,
                            nzp_load, DateCharge.ToShortDateString(), User)
                    }
                };
                        ...
                        if (MainDict.ElementAt(j).Key.ContainsKey(split[0]))
                        {
                            try
                            {
                                if (split[2].Contains("order"))
                                    MainDict.ElementAt(j).Key[split[0]].fields.Add("\"" + split[2] + "\"",
                                        Types[split[3]]);
                ...
                foreach (var dict in MainDict)
                {
                    ...
                    if (dict.Key.Count > 0)
                    {
                        ...
                        ConstrListQuery =
                            ConstrListQuery.Where(x => ConstrListReq.Contains(x.foreign_table)).Distinct().ToList();
                        var ResultList = new List<string>();
                        ResultList.AddRange(
                            ConstrListReq.Where(
                                x =>
                                    !ConstrListQuery.Select(y => y.foreign_table).Contains(x) &&
                                    !(primaryList.Select(z => z.primary_table)).Contains(x)).ToList());
                        var TempList = ConstrListQuery.Select(x => x.foreign_table).Distinct().ToList();
                        while (TempList.Any())
                        {
                            ResultList.AddRange(TempList);
                            TempList =
                                TempList.Where(x => ConstrListQuery.Select(y => y.primary_table).Contains(x))
                                    .Distinct()
                                    .ToList();
                        ...
                        try
                        {
                            ...
                            foreach (var d in ResultList)
                            {
                                try
                                {
                                    var sql = string.Format("INSERT INTO {1}.{0} (",
                                        d.Replace("recalc_" + DateCharge.Year + "_", ""), Schema);
                        ...
                        finally
                        {
                            ...
                            Connection.Execute(sql, null);
                            if (!error)
                                InsertIntoGkhCheck(Convert.ToDateTime(header.chargeDate), schema_code, schema_pref);
                        }
                    ...
                    try
                    {
                        foreach (var file in dict.Key)
                        {
                            ...
                            {
                                errList.Add(string.Format("Ошибка при загрузке файлов:{0}",
                                    ex.Message.Split('\\').Last()));
                            }
                        }
                    }
                    catch (Exception ex)
                ...
                if (Schema.Contains("data"))
                {
                    ...
                    var info1 = new ProcessStartInfo("cmd.exe",
                        string.Format(
                            "/C call \"" + AbsolutePath +
                            "\\gkh_triggers_data_on.bat\" \"{0}\" \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{6}\"",
                            Schema, PsqlPath, Server, Port, Database, User, Password))
                    {
                        WorkingDirectory = Directory.GetCurrentDirectory(),
                        CreateNoWindow = true,
...
/*
                ...
                lock (obj)
                {
                    flink = CreateProtocol(reportList);
                    Connection.Execute("UPDATE sys_imports " +
                                       " SET result='" + flink.Replace("\'", "\'\'").Split('\\').Last() + "', status='" +
                                       "Завершено" +

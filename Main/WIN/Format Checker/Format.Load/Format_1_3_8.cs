...
{
    [Assemble(FormatName = "Характеристики жилого фонда", Version = "1.3.8", RegistrationName = "Format")]
    public class Format20141201 : IFormat
    {
        /// <summary>
        ...
        public class FormatChecker : IFormatChecker
        {
                ...
                try
                {
                         ...
                         checkList["1"][0][1].Trim() == "1.3.8"))
                    {
                        try
                        {
                            ...
                            foreach (var item in checkList)
                            {
                                ...
                                foreach (var itemc in item.Value)
                                {
                                    ...
                                    erList.AddRange(
                                          itemc.Select((t, i) => curTemplate[i].CheckValues(index, t, k, item.Key, sets, checkList))
                                              .Where(str => str != null));
                        ...
                        erList.Add(string.Format(
                            "Формат проверяемого файла ({0}), не совпадает с форматом проверки (1.3.8)",
            ...
            public override Dictionary<string, string> GetTemplatesHead()
            {
                return new Dictionary<string, string>
                {
                    {"1", "ЗАГОЛОВОК ФАЙЛА"},
                    {"2", "ЮРИДИЧЕСКИЕ ЛИЦА"},
                    {"3", "ИНФОРМАЦИЯ О ДОМАХ"},
                    {"4", "ИНФОРМАЦИЯ О ЛИЦЕВЫХ СЧЕТАХ"},
                    {"5", "ИНФОРМАЦИЯ О ДОГОВОРАХ НА ОКАЗАНИЕ ЖКУ"},
                    ...
                    {"12", "ПОКАЗАНИЯ ИНДИВИДУАЛЬНЫХ ПРИБОРОВ УЧЕТА"},
                    ...
                    {"26", "ПЕРЕЧЕНЬ ПРИНЯТЫХ ПАЧЕК ПЛАТЕЖЕЙ ЛИЦЕВОГО СЧЕТА"},
                    {"27", "РЕЕСТР ЛИЦЕВЫХ СЧЕТОВ В СИСТЕМЕ ПОСТАВЩИКА (ПРИ СЛИЯНИИ ДАННЫХ НЕСКОЛЬКИХ ПОСТАВЩИКОВ В ЕДИНЫЙ ЛИЦЕВОЙ СЧЕТ ДЛЯ ПЕЧАТИ ЕПД)"},
                    {"28", "РЕЕСТР ВРЕМЕННО-УБЫВШИХ"},
                    {"29", "БАНКИ"},
                    {"30", "РАСЧЕТНЫЕ СЧЕТА"},
                    {"31", "СОГЛАШЕНИЯ ПО ПЕРЕЧИСЛЕНИЯМ В РАЗРЕЗЕ ДОГОВОРОВ"},
                    {"32", "РАСПРЕДЕЛЕНИЕ ОПЛАТ"},
                    {"33", "ПЕРЕКИДКИ (ИЗМЕНЕНИЕ САЛЬДО)"},
                    {"34", "ИНФОРМАЦИЯ О ЛИЦЕВЫХ СЧЕТАХ, ПРИНАДЛЕЖАЩИХ ПРИБОРУ УЧЕТА"}
                };
            }
                ...
                var allList = new Dictionary<string, List<Template>>
                {
                    ...
                    },
                    {
                        "20", new List<Template>
                        {
                            ...
                            new Template("Дата начала действия значения", Types.DateTime, null, 14, false),
                            new Template("Дата окончания действия значения", Types.DateTime, null, 14, false)
                        }
                    },
                    {
                    ...
                    },
                    {
                        "30", new List<Template>
                        {
                            ...
                            new Template("Уникальный код банка", Types.Int, null, null),
                            new Template("Уникальный код ЮЛ", Types.Int, null, null, true, 1, "2"),
    ...
    [Assemble(FormatName = "Характеристики жилого фонда", Version = "1.3.5.1", RegistrationName = "Format1351")]
    public class Format20150408 : IFormat
    {
        /// <summary>
        /// Класс проверки формата
        /// </summary>
        public class FormatChecker : IFormatChecker
        {
            ...
            public override Returns CheckData(ref object dt)
            {
                ...
                var sets = Template.CreateSets(templateDict, checkList, ref erList);
                sucList.Add(programVersion);
                if (checkList != null && (Template.FormatTypes.Contains(checkList["1"][0][1].Trim()) && checkList["1"][0][1].Trim() == "1.3.5.1"))
                {
                    try
                    {
                        sucList.Add(string.Join("|", checkList["1"][0]));
                ...
                else erList.Add(string.Format("Формат проверяемого файла ({0}), не совпадает с форматом проверки (1.3.5.1)", checkList["1"][0][1].Trim()));
                //Проверка cчетчиков на переход через ноль
                var counters = new[] { new Counters("Показания индивидуальных приборов учета", "12", 1, 3, 4), new Counters("Показания индивидуальных приборов учета", "10", 1, 3, 4) };
                erList.AddRange(counters.Select(c => c.CheckCounters(checkList)));
                erList = erList.Where(x => x != null).ToList();
                if (erList.Count == 0)
                ...
                var allList = new Dictionary<string, List<Template>>
                    {
                        ...
                        },
                        {
                            "4", new List<Template>
                            {
                                ...
                                new Template("Участок",Types.String , null, 60,false),
                                new Template("Код ЮЛ, где лежит паспорт дома",Types.Int , null, null,true,1,"2"),
                                new Template("Этаж квартиры",Types.Int , 1, 56,false)
                            }
                        },
                        {
                            "5", new List<Template>
                        ...
                        },
                        {
                            "22", new List<Template>
                            {
                                ...
                                new Template("Сумма пени",Types.NumericMoney ,null,null,false)
                            }
                        },
                        {
                            "23", new List<Template>
    ...
    [Assemble(FormatName = "Характеристики жилого фонда", Version = "1.3.8.1", RegistrationName = "Format1381")]
    public class Format20150512 : IFormat
    {
        /// <summary>
        /// Класс проверки формата
        /// </summary>
            ...
            public override Returns CheckData(ref object dt)
            {
                ...
                try
                {
                         ...
                         checkList["1"][0][1].Trim() == "1.3.8.1"))
                    {
                        ...
                        erList.Add(string.Format(
                            "Формат проверяемого файла ({0}), не совпадает с форматом проверки (1.3.8.1)",
                            checkList["1"][0][1].Trim()));
                }
                catch (Exception ex)
                {
                ...
                var allList = new Dictionary<string, List<Template>>
                {
                    ...
                    },
                    {
                        "9", new List<Template>
                        {
                            ...
                            new Template("Дополнительные характеристики ОДПУ", Types.String, null, 250, false),
                            new Template("Тип прибора учета", Types.Int, 1, 3),
                            new Template("Дата закрытия счетчика", Types.DateTime,null,null,false)
                        }
                    },
                    {
                        "10", new List<Template>
    ...
    //todo не доступно
    //[Assemble(FormatName = "Характеристики жилого фонда - Не рабочий", Version = "1.3.8.2", RegistrationName = "Format1382")]
    //public class Format20150618 : IFormat
    //{
    //    /// <summary>
    //    /// Класс проверки формата
    //    /// </summary>
    ...
    //                else
    //                    erList.Add(string.Format(
    //                        "Формат проверяемого файла ({0}), не совпадает с форматом проверки (1.3.8.2)",
    //                        checkList["1"][0][1].Trim()));
    //            }
    //            catch (Exception ex)
    //            {
    ...
    //                {"33", "ПЕРЕКИДКИ (ИЗМЕНЕНИЕ САЛЬДО)"},
    //                {"34", "ИНФОРМАЦИЯ О ЛИЦЕВЫХ СЧЕТАХ, ПРИНАДЛЕЖАЩИХ ПРИБОРУ УЧЕТА"},
    //                {"35", "ИНФОРМАЦИЯ О ПРАВОУСТАНАВЛИВАЮЩИХ ДОКУМЕНТАХ СОБСТВЕННОСТИ ИЛИ АРЕНДЫ"},
    //                {"40", "ИНФОРМАЦИЯ О ГРУППАХ НОРМАТИВОВ"},
    //                {"41", "ИНФОРМАЦИЯ О ТАБЛИЦЕ НОРМАТИВА"}
    //            };
    //        }
    ...
    //                },
    //                {
    //                    "35", new List<Template>
    //                    {
    //                        new Template("Номер секции", Types.Int, 0, null),
    //                        new Template("Уникальный код документа собственности", Types.String, null, 20),
    //                        new Template("Номер лицевого счета ", Types.String, null, 20,true, 3, "4"),
    //                        new Template("Уникальный номер гражданина собственника ", Types.Int, null, 20,false),
    //                        new Template("Код юридического лица (собственника / арендатора) ", Types.Int, null, 20,false),
    //                        new Template("Фамилия собственника/ арендатора в документе ", Types.String, null, 30),
    //                        new Template("Имя собственника/ арендатора в документе ", Types.String, null, 30),
    //                        new Template("Отчество собственника/ арендатора в документе ", Types.String, null, 30,false),
    //                        new Template("Дата рождения собственника/ арендатора в документе ", Types.Int, null, 30),
    //                    }
    //                }
    //            };

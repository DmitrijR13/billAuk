...
{
    class ImgLnkSactionsTable:TableSecondLevel
    {
        public ImgLnkSactionsTable(string nameParentTreeViewNode)
        {
        ...
        private const string nodeTreeViewText = "Изображения действий";
        private const string tableName = "img_lnk_s_actions";
        string selectCommand = "";//"WITH selectcreate as (SELECT nzp_img, cur_page, nzp_act, img_url,  created_on, created_by, changed_on, changed_by, user_name as user_name_created "+
//"FROM img_lnk LEFT OUTER JOIN users ON (created_by=id)), "+
// "selectchange AS (SELECT nzp_img, cur_page, nzp_act, img_url,  created_on, created_by, changed_on, changed_by, user_name_created, user_name as user_name_changed "+
//"FROM selectcreate LEFT OUTER JOIN users ON (changed_by=id)) "+              
//"SELECT  nzp_img, selectchange.cur_page, selectchange.nzp_act, selectchange.img_url, selectchange.created_on, selectchange.created_by, "+
//"selectchange.changed_on, selectchange.changed_by,user_name_created, user_name_changed, s_actions.nzp_act||' '|| s_actions.act_name as act_name "+
//"FROM selectchange LEFT OUTER JOIN s_actions ON (selectchange.nzp_act=s_actions.nzp_act);";
        ...
        public override void GetCorrespondTable(DataSet dataSet, NpgsqlDataAdapter adapter, NpgsqlConnection connect, int positionParam)
        {
                ...
                selectCommand = "WITH selectcreate as (SELECT nzp_img, cur_page, nzp_act, img_url, kod, tip, created_on, created_by, changed_on, changed_by, user_name as user_name_created " +
"FROM " + Tables.img_lnk + " LEFT OUTER JOIN " + Tables.users + " ON (created_by=id)), " +
 "selectchange AS (SELECT nzp_img, cur_page, nzp_act, img_url, kod, tip, created_on, created_by, changed_on, changed_by, user_name_created, user_name as user_name_changed " +
"FROM selectcreate LEFT OUTER JOIN " + Tables.users + " ON (changed_by=id)) " +
"SELECT  nzp_img, selectchange.cur_page, selectchange.nzp_act, selectchange.img_url, kod, tip, selectchange.created_on, selectchange.created_by, " +
"selectchange.changed_on, selectchange.changed_by,user_name_created, user_name_changed, s.nzp_act||' '|| s.act_name as act_name " +
"FROM selectchange LEFT OUTER JOIN " + Tables.s_actions + " s ON (selectchange.nzp_act=s.nzp_act) WHERE  selectchange.nzp_act=" + positionParam + " ORDER BY nzp_img; ";
                dataSet.Tables[tableName].Clear();
                adapter.SelectCommand.CommandText = selectCommand;
               // adap.SelectCommand.CommandText = selectCommand;
        ...
        {
            return "WITH selectcreate as (SELECT nzp_img, cur_page, nzp_act, img_url, kod, tip, created_on, created_by, changed_on, changed_by, user_name as user_name_created " +
"FROM " + Tables.img_lnk + " LEFT OUTER JOIN " + Tables.users + " ON (created_by=id)), " +
 "selectchange AS (SELECT nzp_img, cur_page, nzp_act, img_url, kod, tip, created_on, created_by, changed_on, changed_by, user_name_created, user_name as user_name_changed " +
"FROM selectcreate LEFT OUTER JOIN " + Tables.users + " ON (changed_by=id)) " +
"SELECT  nzp_img, selectchange.cur_page, selectchange.nzp_act, selectchange.img_url, kod, tip, selectchange.created_on, selectchange.created_by, " +
"selectchange.changed_on, selectchange.changed_by,user_name_created, user_name_changed, s.nzp_act||' '|| s.act_name as act_name " +
"FROM selectchange LEFT OUTER JOIN " + Tables.s_actions + " s ON (selectchange.nzp_act=s.nzp_act) WHERE  selectchange.nzp_act=" + Position + " ORDER BY nzp_img; ";
        }
        ...
        public override void CreateColumns()
        {
            ...
            columnCollection[5] = CreateDataGridViewColumn.CreateTextBoxColumn(tableName, "nzp_act", "Действие", true, false);
            columnCollection[4] = CreateDataGridViewColumn.CreateTextBoxColumn(tableName, "kod", "Действие", true, false);
            columnCollection[3] = CreateDataGridViewColumn.CreateTextBoxColumn(tableName, "tip", "Действие", true, false);
            //ColumnCollection[3] = CreateDataGridViewColumn.CreateTextBoxColumn(tableName, "tip", "Тип", false, true);
            columnCollection[2] = CreateDataGridViewColumn.CreateComboBoxColumn<string>("Текущая страница", "cur_page");
            observers.Add(new ComboBoxBindingSource(columnCollection[2], true, "", false, true));
            columnCollection[1] = CreateDataGridViewColumn.CreateTextBoxColumn(tableName, "act_name", "Наименование действия", true, false);
        ...
        public override bool Save(NpgsqlConnection connect, NpgsqlDataAdapter adapter, DataTable dt)
        {
            ...
            // Обновление данных
            adapter.UpdateCommand = new NpgsqlCommand("UPDATE " + Tables.img_lnk + " SET nzp_img=@nzp_img, cur_page=@cur_page,  nzp_act=@nzp_act, kod=@kod, tip=@tip, img_url=@img_url, " +
            ...
            paramter.SourceVersion = DataRowVersion.Original;
            // Вставка строк 
            //nzp_img,nextval('img_lnk_nzp_img_seq'::regclass),
            adapter.InsertCommand = new NpgsqlCommand("Insert into " + Tables.img_lnk + " ( cur_page,  nzp_act, img_url, kod, tip, created_on, created_by, changed_on, changed_by) " +
            "values ( @cur_page,  @nzp_act, @img_url, @kod, @tip, @created_on, @created_by, @changed_on, @changed_by); ", connect);
        ...
        public override void SetDefaultValuesAfterRowAdded(DataGridView dgv, int index)
        {
            ...
            dgv.Rows[index].Cells["nzp_act"].Value = position;
            ...
            dgv.Rows[index].Cells["tip"].Value = 2;
        }
        ...
        public override void SetDefaultValuesAfterCellChange(DataGridView dgv, int row, int column)
        {
            ...
            if (dataGridViewColumn != null && dataGridViewColumn.Index == column)
            {
                ...
                GenerateScript.AddVal(Tables.img_lnk, valColumns, whereDict, idColVal, typescr, (int)dgv.Rows[row].Cells["nzp_act"].Value);
                return;
            }

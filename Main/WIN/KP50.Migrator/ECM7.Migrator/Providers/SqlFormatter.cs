...
namespace KP50.DataBase.Migrator.Providers
{
    ...
    {
        protected readonly char tableDelimiter;
        ...
        public SqlFormatter(char TableDelimiter, Func<object, string> converter)
        {
            Require.IsNotNull(TableDelimiter, "Isn't set table delimiter.");
                ...
                if (ufmt == "NAME")
                {
                    if (arg is SchemaQualifiedObjectName)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(typedArg.Schema))
                        {
                            ...
                            return string.Format("{0}{1}{2}", schema, tableDelimiter, name);
                        }

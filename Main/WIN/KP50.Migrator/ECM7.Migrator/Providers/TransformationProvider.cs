...
using System.Collections.ObjectModel;
    ...
    public abstract class TransformationProvider : SqlRunner, ITransformationProvider
    {
        ...
        public SchemaQualifiedObjectName SCHEMA_INFO_TABLE { get; set; }
        ...
        protected ReadOnlyCollection<TableMap> basesMap = null;

        protected TransformationProvider(IDbConnection connection)
            : base(connection)
        {
            base.CommandTimeout = 300;
            SCHEMA_INFO_TABLE = new SchemaQualifiedObjectName() { Name = "SchemaInfo", Schema = null };
            NeedQuotesForNames = true;
            sqlFormatProvider = new SqlFormatter(TableDelimiter, GetQuotedName);
        ...
        #region KP50
        public List<string> GetBasesList(DataBase database, string CentralPref) 
        {
            List<string> lstKernels = new List<string>();
            List<string> lstWhere = new List<string>();
            switch (database)
            {
                case DataBase.CentralBank:
                    lstKernels.Add(string.Format("{0}_kernel", CentralPref));
                    break;
                case DataBase.LocalBank:
                    GetLocalPrefixes(CentralPref).ForEach(pref => lstKernels.Add(string.Format("{0}_kernel", pref)));
                    break;
                default:
                    string strSqlQuery = FormatSql("SELECT {0} FROM {1:NAME}",
                        "dbname", new SchemaQualifiedObjectName() { Name = "s_baselist", Schema = String.Format("{0}_kernel", CentralPref) });
                    if (database.HasFlag(DataBase.Charge)) lstWhere.Add("idtype = 1");
                    if (database.HasFlag(DataBase.Fin)) lstWhere.Add("idtype = 4");
                    if (database.HasFlag(DataBase.Web)) lstWhere.Add("idtype = 255");
                    if (lstWhere.Count > 0) strSqlQuery += " WHERE " + string.Join(" OR ", lstWhere);
                    using (IDataReader reader = ExecuteReader(strSqlQuery))
                        while (reader.Read()) lstKernels.Add(reader.GetString(0).Trim());
                    break;
            }
            return lstKernels;
        }

        public List<string> GetLocalPrefixes(string CentralPref)
        {
            List<string> lstPrefixes = new List<string>();
            string strSqlQuery = FormatSql("SELECT {0} FROM {1:NAME} WHERE {2} = 1",
                "bd_kernel", new SchemaQualifiedObjectName() { Name = "s_point", Schema = String.Format("{0}_kernel", CentralPref) }, "nzp_graj");
            using (IDataReader reader = ExecuteReader(strSqlQuery)) while (reader.Read()) lstPrefixes.Add(reader.GetString(0).Trim());
            return lstPrefixes;
        }

        public virtual void SetSchema(string SchemaName)
        {
            ExecuteNonQuery("DATABASE " + SchemaName);
        }
        #endregion
        ...
        public virtual char TableDelimiter { get { return '.'; } }
        ...
        {
            // ñïèñîê êîëîíîê, âõîäÿùèõ â ïåðâè÷íûé êëþ÷
            ...
            // SQL äëÿ ñîñòàâíîãî ïåðâè÷íîãî êëþ÷à
        ...
        public abstract void AddSequence(string schema, string name);
        ...
        public abstract bool SequenceExists(string schema, string name);
        ...
        public abstract void RemoveSequence(string schema, string name);
        ...
        public abstract bool ProcedureExists(string SchemaName, string Name);
        ...
        public IList<TableMap> GetBasesMap()
        {
            var result = new List<TableMap>();
            string sql = FormatSql("SELECT {0:NAME}, {1:NAME}, {2:NAME} FROM {3:NAME}", 
                "Version", "AssemblyKey", "Prefix", SCHEMA_INFO_TABLE);

            using (IDataReader reader = ExecuteReader(sql))
            {
                while (reader.Read())
                {
                    TableMap existed = null;
                    var mappingObj = new TableMap(reader.GetString(1), reader.GetString(2));
                    if ((existed = result.FirstOrDefault(x => x.AssemblyKey == mappingObj.AssemblyKey && x.Table == mappingObj.Table)) != null)
                        existed.Versions.Add(reader.GetInt64(0));
                    else { mappingObj.Versions.Add(reader.GetInt64(0)); result.Add(mappingObj); }
                }
            }
            return result;
        }
        ...
        /// </summary>
        public List<long> GetAppliedMigrations(string SchemaName, string key = "")
        {
            if (basesMap == null) basesMap = new ReadOnlyCollection<TableMap>(GetBasesMap());
            Require.IsNotNull(key, "Не указан ключ миграциий");
            var appliedMigrations = new List<long>();
            var cache = basesMap.FirstOrDefault(x => x.AssemblyKey == key && x.Table == SchemaName);
            if (cache != null) appliedMigrations.AddRange(cache.Versions);
        ...
        public void MigrationApplied(long version, string key, string Prefix)
        {
            CreateSchemaInfoTable();
            Insert(SCHEMA_INFO_TABLE, new[] { "Version", "AssemblyKey", "Prefix" }, new[] { version.ToString(), key, string.IsNullOrWhiteSpace(Prefix) ? "web" : Prefix.Replace("'", "''").ToLower() });
        }
        ...
        /// <param name="key">Key of migration series</param>
        public void MigrationUnApplied(long version, string key, string Prefix)
        {
            CreateSchemaInfoTable();

            string whereSql = FormatSql("{0:NAME} = {1} AND {2:NAME} = '{3}' AND {4:NAME} = '{5}'",
              "Version", version, "AssemblyKey", key, "Prefix", string.IsNullOrWhiteSpace(Prefix) ? "web" : Prefix.Replace("'", "''").ToLower());
        ...
        protected void CreateSchemaInfoTable()
        {
            ...
            else
            {
                if (!ColumnExists(SCHEMA_INFO_TABLE, "AssemblyKey"))
                {
                    ...
                    {
                        UpdateSchemaInfo.Update2To4(this);
                    }
                    else
                    {
                        UpdateSchemaInfo.Update1To4(this);
                    }
                }
                if (!ColumnExists(SCHEMA_INFO_TABLE, "UpdateDate"))
                    UpdateSchemaInfo.Update3To4(this);
            }
        }

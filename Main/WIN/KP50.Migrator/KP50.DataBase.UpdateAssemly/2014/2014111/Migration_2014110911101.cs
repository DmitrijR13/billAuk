...
namespace KP50.DataBase.UpdateAssembly._2014._2014102
{
    ...
    // DD   - Day
    // VVV  - Version
    [Migration(2014110911101, MigrateDataBase.CentralBank)]
    public class Migration_2014110911101_CentralBank : Migration
    {
        public override void Apply()
        {
            ...
            if (Database.ProviderName == "PostgreSQL")
            {
                ...
                if (!Database.TableExists(peni_provodki_arch))
                {
                    ...
                    if (Database.TableExists(peni_provodki_arch))
                    {
                        ...
                        Database.AddIndex("ix3_peni_provodki_arch", false, peni_provodki_arch, "id", "nzp_wp",
                            "date_obligation", "date_prov", "date_arch");
                ...
                #region Справочник типов действий по пени
                SetSchema(Bank.Kernel);
                ...
                if (!Database.TableExists(peni_actions_type))
                {
                    ...
                    if (Database.TableExists(peni_actions_type))
                    {
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "1", "Запись проводок" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "2", "Архивация проводок" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "3", "Запись задолженностей" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "4", "Запись пени" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "5", "Запись проводок при перерасчете" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "6", "Погашение задолженностей при перерасчете" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "7", "Запись задолженностей при перерасчете" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "8", "Погашение пени при перерасчете" });
                        Database.Insert(peni_actions_type, new string[] { "id", "type" }, new string[] { "9", "Запись пени при перерасчете" });
                    }
                }
                #endregion

                #region Реестр действий по пени
                SetSchema(Bank.Data);
                ...
                {
                    Database.AddTable(peni_actions,
                     new Column("id", DbType.Int32, ColumnProperty.PrimaryKeyWithIdentity | ColumnProperty.NotNull),
                     new Column("peni_actions_type_id", DbType.Int32, ColumnProperty.NotNull),
                     new Column("created_by", DbType.Int32),
                ...
                {
                    Database.ChangeColumn(peni_no_calc, "date_from", DbType.Date, true);
                    Database.ChangeColumn(peni_no_calc, "date_to", DbType.Date, true);
                }
        ...
        {
            Console.WriteLine("Applying 2014110911101: Migration 2014110911101 Bank: " + bank);
        }

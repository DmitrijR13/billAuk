...
namespace KP50.DataBase.Migrator.Framework
{
    ...
    {
        char TableDelimiter { get; }
        IDbConnection Connection { get; }
        int CommandTimeout { get; set; }
        bool NeedQuotesForNames { get; set; }
        List<string> GetLocalPrefixes(string CentralPref);

        List<string> GetBasesList(DataBase database, string CentralPref);

        void SetSchema(string SchemaName);

        string ProviderName { get; }

        SchemaQualifiedObjectName SCHEMA_INFO_TABLE { get; set; }
        
        bool ProcedureExists(string SchemaName, string Name);

        /// <summary>
        /// The list of Migrations currently applied to the database.
        /// </summary>
        List<long> GetAppliedMigrations(string SchemaName, string key = "");
        ...
        /// </summary>
        /// <param name="indexName">Íàçâàíèå èíäåêñà</param>
        /// <param name="tableName">Íàçâàíèå òàáëèöû</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="table">The name of the table that will get the new column</param>
        /// <param name="column">Íàçâàíèå èçìåíÿåìîé êîëîíêè òàáëèöû</param>
        /// <param name="columnType">Íîâûé òèï êîëîíêè</param>
        /// <param name="notNull">Ïðèçíàê: çàïðåùåíî çíà÷åíèå NULL</param>
        void ChangeColumn(SchemaQualifiedObjectName table, string column, ColumnType columnType, bool notNull);

        /// <summary>
        /// Èçìåíåíèå çíà÷åíèÿ ïî óìîë÷àíèþ
        /// </summary>
        /// <param name="table">Íàçâàíèå òàáëèöû</param>
        /// <param name="column">Íàçâàíèå êîëîíêè</param>
        /// <param name="newDefaultValue">Íîâîå çíà÷åíèå ïî óìîë÷àíèþ</param>
        ...
        void MigrationApplied(long version, string key, string Prefix);
        ...
        /// <param name="key">Key of migration series</param>
        void MigrationUnApplied(long version, string key, string Prefix);
        ...
        bool SequenceExists(string schema, string name);

        void AddSequence(string schema, string name);

        void RemoveSequence(string schema, string name);

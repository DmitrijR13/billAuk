...
namespace KP50.DataBase.Migrator.Framework
{
    ...
    public abstract class Migration : IMigration
    {
        ...
        public string CentralPrefix { get; set; }

        public string CentralKernel { get; set; }

        public string CentralData { get; set; }

        public virtual void SetSchema(Bank bank)
        {
            if (CurrentPrefix != null)
            {
                CurrentSchema = string.Format("{0}_{1}", CurrentPrefix, Enum.GetName(typeof(Bank), bank)).ToLower();
                Database.SetSchema(string.Format("{0}_{1}", CurrentPrefix, Enum.GetName(typeof(Bank), bank)).ToLower());
            }
        }

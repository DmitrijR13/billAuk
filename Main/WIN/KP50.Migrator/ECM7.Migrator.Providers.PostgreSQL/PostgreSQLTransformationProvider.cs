...
namespace KP50.DataBase.Migrator.Providers.PostgreSQL
{
    /// <summary>
    ...
    public class PostgreSQLTransformationProvider : TransformationProvider
    {
            ...
            : base(connection)
        {
            ...
            typeMap.Put(DbType.DateTime2, "timestamp without time zone");
        ...
        public override string ProviderName { get { return "PostgreSQL"; } }
        #endregion

        #region custom sql
        public override void SetSchema(string SchemaName)
        {
            ExecuteNonQuery(String.Format("SET search_path TO '{0}'", SchemaName));
        }
        ...
        {
            using (IDataReader reader = ExecuteReader(FormatSql(
                "SELECT proname FROM pg_catalog.pg_proc p LEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname = '{0}' AND proname = '{1}'",
                SchemaName, Name)))
            {
                return reader.Read();
            }
        ...
        {
            string sql = string.Format("SELECT c.relname FROM pg_catalog.pg_class c INNER JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = '{0}' AND n.nspname = '{1}';", name, schema);
            using (IDataReader reader = ExecuteReader(sql))
            {
                return reader.Read();
            }
        ...
        {
            ExecuteNonQuery(string.Format("DROP SEQUENCE {0}.{1} CASCADE", schema, name));
        }

...
//                          " max(year(dat_po)*12+month(dat_po)) as month_po" +
            ...
            while (reader.Read())
            {
                if (reader["pref"] != null)
                {
                    ...
                    sql = " insert into t1 (nzp_geu, nzp_dom, nzp_kvar, is_device, nzp_frm, " +
                          ...
                          " sum(case when real_charge<0 and nzp_serv in (9,513) then real_charge else 0 end) +" +
                          " sum(case when reval<0 and nzp_serv in (9,513) then reval else 0 end) as reval_k," +
                          " sum(case when real_charge>0 and nzp_serv in (9,513) then real_charge else 0 end) +" +
                          " sum(case when reval>0 and nzp_serv in (9,513) then reval else 0 end) as reval_d," +

                          " sum(case when nzp_serv=14 then sum_nedop else 0 end) as sum_nedop_kub," +
                          " sum(case when real_charge<0 and nzp_serv in (14,514) then real_charge else 0 end) +" +
                          " sum(case when reval<0 and nzp_serv in (14,514) then reval else 0 end) as reval_k_kub," +
                          " sum(case when real_charge>0 and nzp_serv in (14,514) then real_charge else 0 end) +" +
                          " sum(case when reval>0 and nzp_serv in (14,514)then reval else 0 end) as reval_d_kub" +
...
#warning PG
                    ...
                    sql = " update t1 set tarif_gkal =" + DBManager.sNvlWord + "((select max(tarif) " +
                          " from t_tar where t1.nzp_dom=t_tar.nzp_dom   ),0) where nvl(tarif_gkal,0) = 0 ";
                    ExecSQL(connDb, sql, true);

                    sql = " update t1 set tarif_hvs =" + DBManager.sNvlWord + "((select max(tarif_hvs) " +
                      " from t_tar where t1.nzp_dom=t_tar.nzp_dom   ),0) where nvl(tarif_hvs,0) = 0 ";
                    ExecSQL(connDb, sql, true);

                    sql = " update t1 set gkal_koef =" + DBManager.sNvlWord + "((select max(gkal_koef) " +
                         " from t_tar where t1.nzp_dom=t_tar.nzp_dom   ),0) where nvl(gkal_koef,0) = 0 ";
                    ExecSQL(connDb, sql, true);

                    sql = " update t1 set tarif_gkal =" + DBManager.sNvlWord + "((select max(tarif) " +
                         " from t_tar ),0) where nvl(tarif_gkal,0) = 0 ";
                    ExecSQL(connDb, sql, true);
                    sql = " update t1 set tarif_hvs =" + DBManager.sNvlWord + "((select max(tarif_hvs) " +
                    " from t_tar ),0) where nvl(tarif_hvs,0) = 0 ";
                    ExecSQL(connDb, sql, true);

                    sql = " update t1 set gkal_koef =" + DBManager.sNvlWord + "((select max(gkal_koef) " +
                        " from t_tar),0) where nvl(gkal_koef,0) = 0 ";
                    ExecSQL(connDb, sql, true);

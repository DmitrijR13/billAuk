        ...
        public DataTable GetSpravSuppNachHar(Prm prm_, out Returns ret, string Nzp_user)
        {
            ...
            while (reader.Read())
            {
                if (reader["pref"] != null)
                {
...
#if PG
                    sql2.Append(" create unlogged table selected_kvars( ");
                    ...
                    sql2.Append(where_supp + where_supp);
                    ...
                    sql3 = "update t_nachdom set sum_nedop_cor = (select  " +
                          " -sum(sum_rcl) " +
                          " from " + db_charge + DBManager.tableDelimiter + "perekidka a " +
                          " where a.nzp_kvar=t_nachdom.nzp_kvar and type_rcl = 101 and month_=" + prm_.month_.ToString("00") +
...
#else
                    sql2.Append(" update t_nachdom set (pl_kvar, count_gil) = ((select max(squ), max(gil) ");
                    sql2.Append(" from  " + db_charge + ":calc_gku_" + prm_.month_.ToString("00") + " d ");
                    sql2.Append(" where d.nzp_kvar=t_nachdom.nzp_kvar and d.tarif>0 ");
                    sql2.Append(" and t_nachdom.nzp_serv=d.nzp_serv and t_nachdom.nzp_supp=d.nzp_supp ))");
#endif
...
//                    ExecSQL(conn_db, sql2.ToString(), true);
                    ...
                    sql2.Append(" into unlogged t_nedop  from " + pref + "_data.nedop_kvar a, selected_kvars b ");
...
#else
                    sql2.Append(" select a.nzp_kvar, min(year(dat_s)*12+month(dat_s)) as month_s,  max(year(dat_po)*12+month(dat_po)) as month_po");
                    ...
                    sql2.Append(" from t_nachdom t INNER JOIN " + pref + DBManager.sKernelAliasRest + "supplier s ON s.nzp_supp = t.nzp_supp ");
...
#if PG
                    sql2.Append(" select nzp_kvar, nzp_supp ");
                    sql2.Append(" into unlogged t12  from t_nachdom a ");
                    ...
                    sql2.Append(" into unlogged t_nachdom2 from  t_nachdom ");
                    sql2.Append(" group by 1,2 ");
            ...
            sql2.Append(" -sum(coalesce(reval_k,0)+coalesce(real_charge_k,0)) as reval_k , ");
            sql2.Append(" sum(coalesce(reval_d,0)+coalesce(real_charge_d,0)) as reval_d , ");

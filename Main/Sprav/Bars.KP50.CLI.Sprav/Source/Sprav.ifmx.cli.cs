   ...
   IDbCommand cmd = DBManager.newDbCommand(
        ...
        //----------------------------------------------------------------------
        {
            ...
            */
            ...
            ExecSQL(connectionID, "drop table t_ps", false);
            
            Returns ret = ExecSQL(connectionID, "create temp table t_ps (cur_page INTEGER, page_url INTEGER, up_kod INTEGER, sort_kod INTEGER, img_url char(255)) " + sUnlogTempTable, true);
            if (!ret.result) return;

            ret = ExecSQL(connectionID, "insert into t_ps Select cur_page, page_url, up_kod, sort_kod, '' From pages_show"+
#if PG
                ""//" Where CAST(sort_kod as text)||CAST(up_kod as text)||CAST(page_url as text)||CAST(cur_page as text)||'-'||CAST(nzp_psh as text)||'pages_show' = sign "
#else
                ""//" Where sort_kod||up_kod||page_url||cur_page||'-'||nzp_psh||'pages_show' = decrypt_char(sign) "
#endif
            , true);
            ...
            ret = ExecSQL(connectionID, "update t_ps set img_url = (select max(img_url) from img_lnk where tip = 1 and kod = t_ps.page_url and (cur_page = t_ps.cur_page or cur_page = 0))", true);
            if (!ret.result) return;

            ret = ExecRead(connectionID, out reader, "select cur_page, page_url, up_kod, sort_kod, img_url from t_ps order by cur_page, up_kod, sort_kod", true);
            if (!ret.result) return;

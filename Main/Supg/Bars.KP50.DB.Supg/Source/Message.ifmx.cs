...
namespace STCLINE.KP50.DataBase
{
  ...
  sql = " Insert into " + tXX_mess_full + " (" +
                      ...
                      " Select s.message_id," +
                        ...
                        " s.created_on, (select u.comment From " + Points.Pref + "_data.users u where u.nzp_user = s.created_by) creator," +
                        " s.sended_on, (select u.comment From " + Points.Pref + "_data.users u where u.nzp_user = s.sended_by) sender," + 
...
string sql = " select * from " + Points.Pref + "_msg.s_message_status ";
        ...
        public List<SmsReceiver> GetSmsReceiver(out Returns ret)
        {
            ...
            if (!ret.result) return null;
#if PG
            string sql = " select * from " + Points.Pref + "_supg.s_receiver r order by r.receiver, r.mobile_phone ";
#else
 ...
 sql = "Update " + Points.Pref + "_supg._s_receiver Set " +
            ...
            {
#if PG
   sql = "Insert into " + Points.Pref + "_supg.s_receiver (receiver, mobile_phone, post) values(" +

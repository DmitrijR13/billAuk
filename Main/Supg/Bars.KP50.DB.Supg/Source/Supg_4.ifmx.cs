        ...
        public bool NedopUnload(Journal finder, out Returns ret)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Archive.GetInstance()
                        .Compress(arcfilename,
                            new string[] { fullfname }, true);
                   

                    if (InputOutput.useFtp)
                        arcfilename = InputOutput.SaveInputFile(arcfilename);
                ...
                sql = " update " + Points.Pref + sSupgAliasRest + "jrn_upg_nedop set exc_path = \'" + arcfilename + "\' where no=" + finder.number;
...
if (spls_from == "")
   ...
   string sql_count = "select count(*) from " +
            ...
            sql = "select distinct " +
                    ...
                    "and (o.nzp_dom=d.nzp_dom or o.nzp_dom=0) " +
                    "and (o.nzp_kvar=k.nzp_kvar or o.nzp_kvar=0)) " +
                    "order by p.dat_po desc LIMIT " + finder.skip + " OFFSET " + finder.rows;
#else
            sql = "select skip " + finder.skip + " first " + finder.rows + " distinct " +
                    " p.nzp_act, " +
                    " p.number, " +
                    " p._date, " +
                    " p.plan_number, " +
...
ExecSQL(con_db, " Drop table " + temp_table_name + "; ", false);
 ...
 sql.Append(" create temp table " + temp_table_name + " (" +
                ...
                sql.Append(
                                           ...
                                           " LEFT OUTER JOIN " + Points.Pref + "_data.dom d ON K .nzp_dom = d.nzp_dom " +
                                           " LEFT OUTER JOIN " + Points.Pref + "_data.s_ulica u ON d.nzp_ul = u.nzp_ul " +
                                           " LEFT OUTER JOIN PUBLIC.t" + finder.nzp_user + "_supg s ON z.nzp_zvk = s.nzp_zvk;");
#else
 sql.Append(
                            " insert  into  " + temp_table_name + " (nzp_zvk, zvk_date, addr, demand_name, comment, phone, nzp_slug, r_comment, result_comment) " +
                            " select " +

...
update pages_show set sign = sort_kod||up_kod||page_url||cur_page||'-'||nzp_psh||'pages_show'
where (cur_page = 333 and page_url = 5) or (cur_page = 5 and page_url = 333);

update role_pages set sign = nzp_role||nzp_page||'-'||id||'role_pages'
where nzp_role = 30;

update role_actions set sign = nzp_role||nzp_page||nzp_act||'-'||id||'role_actions' 
where nzp_role = 30;

-- шифрование
update pages_show   set sign = encrypt_aes(sign) where (cur_page = 333 and page_url = 5) or (cur_page = 5 and page_url = 333);
update role_pages   set sign = encrypt_aes(sign) where nzp_role = 30;
update role_actions set sign = encrypt_aes(sign) where nzp_role = 30;
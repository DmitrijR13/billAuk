...
namespace STCLINE.KP50.DataBase    
{
    ...
    public class DbCalcPack : DbCalcClient
    {
...
sql = "update  " + baseFin + ":pack set flag = 11, sum_pack = (select sum(a.g_sum_ls) from " + baseFin + ":pack_ls a WHERE a.nzp_pack = " + baseFin + ":pack.nzp_pack), sum_rasp = 0, sum_nrasp = " + "(select sum(a.g_sum_ls) from " + baseFin + ":pack_ls a WHERE a.nzp_pack = " + baseFin + ":pack.nzp_pack), " +
        ...
        {
            string sql_text = "insert into " + Points.Pref + "_data"+tableDelimiter+"sys_events(date_, nzp_user, nzp_dict_event, nzp, note) values "+
                "(" + sCurDateTime + ",(select nzp_user from " + Points.Pref + "_data" + tableDelimiter + "users where web_user = " + nzp_user + ")," + nzp_dict + "," + nzp + ",'" + note.Replace("'", "") + "') ";

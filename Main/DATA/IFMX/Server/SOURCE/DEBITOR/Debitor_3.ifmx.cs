    ...
    public partial class Debitor : DataBaseHead
    {
        ...
        public List<Debt> GetDebitors(DebtFinder finder, out Returns ret)
        {
            ...
            try
            {
                ...
                sql.Append("SELECT s.mark, s.area, s.nzp_debt, s.adr, s.fio, s.phone, s.debt_money, s.pref, s.nzp_kvar, s.is_priv, d.nzp_deal ");
                sql.Append("FROM public." + tXX_debt + " s ");
                sql.Append("LEFT JOIN " + Points.Pref + "_debt.deal d ON ");
                sql.Append("d.nzp_kvar = s.nzp_kvar ");
                    ...
                    foreach (_OrderingField order in finder.orderings)
                    {
                        try
                        {
                            ...
                            if (info != null)
                            {
                                ...
                                else if (info.Name == "deal") property = "d.nzp_deal";
                ...
                sql.Remove(0, sql.Length);
                sql.Append("SELECT COUNT(*) FROM public." + tXX_debt + " s ");
                sql.Append("LEFT JOIN " + Points.Pref + "_debt.deal d ON ");
                sql.Append("d.nzp_kvar = s.nzp_kvar ");

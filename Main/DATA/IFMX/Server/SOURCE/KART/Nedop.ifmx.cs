...
if (!TempTableInWebCashe(conn_db, finder.pref + "_data.nedop_kvar"))
            ...
            string tXX_nedop_full = conn_web.Database + ".public."+ tXX_nedop;
#else
            string tXX_nedop = "t" + Convert.ToString(finder.nzp_user) + "_nedop";
            ...
            sql.Append( services +", "+ finder.pref + "_data.nedop_kvar n "+
                        ...
                        " left outer join " + finder.pref + "_data.users u on n.nzp_user = u.nzp_user " +
        ...
        public List<Nedop> GetNedop(Nedop finder, out Returns ret)
        {
...
#if PG
            if (!TempTableInWebCashe(conn_db, finder.pref + "_data.users"))
            {
                ret.result = false;
                ret.text = "Данные о недопоставках временно не доступны";
                ret.tag = -1;
            ...
            string tXX_nedop = "t" + Convert.ToString(finder.nzp_user) + "_nedop n";
                ...
                while (reader.Read())
                {
                    ...
                    if (i == 1)
                    {
                        ...
                        if (TempTableInWebCashe(conn_db, nedop_kvar_full))
                        {
                            ...
                            sql.Append(" left outer join " + finder.pref + "_data.users u on n.user_block = u.nzp_user");                            
        ...
        private List<Nedop> GetNedopDom(Nedop finder, out Returns ret)
        {
            ...
            string groupBy = "";
            if (finder.nzp_dom > 0)
                groupBy += " Group by nzp_serv, service, nzp_supp, supplier, dat_s, dat_po, tn, nzp_kind, kind";
            ...
            try
            {
                #region определить число записей
                int total_record_count = 0;
                ret = ExecRead(conn_web, out reader, "Select nzp_serv, service, nzp_supp, supplier, dat_s, dat_po, tn, nzp_kind, kind From " + tXX_nedop + sWhere + groupBy, true);
                if (!ret.result)
                {
                    conn_web.Close();
                    return null;
                ...
                sql.Append("Select nzp_serv, service, nzp_supp, supplier, dat_s, dat_po, tn, nzp_kind, kind, count(*) as cnt_ls");
...
#if PG
                ret = ExecSQL(conn_db, "select distinct nzp_serv into unlogged tarif_services from " + finder.pref + "_data.tarif where is_actual <> 100 " + dop + " ; ", true);
#else
                ret = ExecSQL(conn_db, "select distinct nzp_serv from " + finder.pref + "_data:tarif where is_actual <> 100 " + dop + " into temp tarif_services; ", true);
#endif
                if (!ret.result)
                {
                    conn_db.Close();
        ...
        public List<NedopType> GetNedopTypeForNedop(Nedop finder, out Returns ret)
        {
...
#if PG
            string sql = "select nt.name, nt.nzp_kind, nt.is_param from " + finder.pref + "_data.upg_s_kind_nedop nt" +
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    nedoptype.key = nedoptype.nzp_kind + "_" + nedoptype.is_param;
                    list.Add(nedoptype);
                }
                reader.Close();
            }
...
#else
            string strSqlQuiery = String.Format("SELECT nzp_serv_slave FROM {0}_data:dep_servs WHERE nzp_serv = {1} AND is_actual = 1 AND {2} >= dat_s, {3} < dat_po;", Points.Pref, finder.nzp_serv, Utils.EDateNull(finder.dat_s), Utils.EDateNull(finder.dat_po));
#endif
        ...
        public List<NedopInfo> PlannedWorksList(int nzp_user, enSrvOper en, out Returns ret)
        {
            ...
            try
            {
                ...
                {
                    sql = "select distinct year(a._date) ydate," + Points.Pref + "_data. sortnum(a.plan_number) a1, a.number, a.plan_date, a.nzp_act, " +
                            "a.comment, a.dat_s, a.dat_po, " +
                            "s.name_supp " +
                            "from " + Points.Pref + "_supg.act a, " + con_web.Database + ".public" + ".supplier s , " + tXX_supg_acts_full + " t " +
                            "where " +
        ...
        public List<NedopInfo> GetRepNedopList(int nzp_user, int nzp_jrn, out Returns ret)
        {
            ...
            try
            {
                ...
                sql = "select distinct " +
                                      ...
                                      "a.number, a._date, " +
                                      "(case when a.dat_s > coalesce(g.d_begin,public.mdy(1,1,1900)) then a.dat_s else EXTEND(g.d_begin, year to hour) end) dat_s, " +
                        ...
                        if (item.ctp == "1")
                        {
                            sql = sql_fields +
                                    "from " + Points.Pref + "_data.s_ulica u, " +
                                             Points.Pref + "_data.s_rajon r, " +
                                             Points.Pref + "_data.dom dleft outer join(" + Points.Pref + "_data.s_geu g), " + Points.Pref + "_data.kvar k " +

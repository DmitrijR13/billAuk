	...
	public partial class DbAdres : DbAdresKernel
	{
		...
		public GetSelectListDomInfo GetSelectListDomInfo(Finder finder, out Returns ret)
		{
...
#if PG
				sql.Append("select distinct a.area from " + tXX_spdom_full + " d, " + Points.Pref + "_data.kvar k, " +
						   points.pref + "_data.prm_3 p3, " + Points.Pref + "_data.s_area a ");
				sql.Append("where d.nzp_dom = k.nzp_dom and k.nzp_area=a.nzp_area and d.mark=1 and p3.nzp=k.nzp_kvar ");
				sql.Append(" and " + dat_s +
						   " between p3.dat_s and p3.dat_po and p3.is_actual<>100 and (p3.nzp_prm=51 and p3.val_prm='1') ");
#else
				sql.Append("select unique a.area from " + tXX_spdom_full + " d, " + Points.Pref + "_data:kvar k, " + points.pref + "_data:prm_3 p3, " + Points.Pref + "_data:s_area a ");
				sql.Append("where d.nzp_dom = k.nzp_dom and k.nzp_area=a.nzp_area and d.mark=1 and p3.nzp=k.nzp_kvar ");
				sql.Append(" and " + dat_s + " between p3.dat_s and p3.dat_po and p3.is_actual<>100 and (p3.nzp_prm=51 and p3.val_prm='1') ");
#endif
			...
			foreach (var points in prefixs)
			{
				sql.Remove(0, sql.Length);
#if PG
				sql.Append(" select count(*) from " + tXX_spdom_full + " d, " + points.pref + "_data.kvar k, " +
						   points.pref + "_data.prm_3 p3 ");
				sql.Append(" where  d.nzp_dom = k.nzp_dom  and d.mark=1 and p3.nzp=k.nzp_kvar ");
				sql.Append(" and " + dat_s +
						   " between p3.dat_s and p3.dat_po and p3.is_actual<>100 and (p3.nzp_prm=51 and p3.val_prm='1') ");
#else
				sql.Append(" select count(*) from " + tXX_spdom_full + " d, " + points.pref + "_data:kvar k, " + points.pref + "_data:prm_3 p3 ");
				sql.Append(" where  d.nzp_dom = k.nzp_dom  and d.mark=1 and p3.nzp=k.nzp_kvar ");
				sql.Append(" and " + dat_s + " between p3.dat_s and p3.dat_po and p3.is_actual<>100 and (p3.nzp_prm=51 and p3.val_prm='1') ");
#endif
		...
		public List<Dom> LoadDom(Dom finder, out Returns ret)
		{
			...
			string sql = "select count(*) from " + tables.dom + " d, " + tables.ulica + " u " +
						 " where d.nzp_ul = u.nzp_ul" + where;
			...
			sql = " select d.nzp_dom, d.ndom, d.nzp_ul, d.nkor, u.ulica from " + tables.dom + " d, " + tables.ulica +
				  " u " +
				  " where d.nzp_ul = u.nzp_ul" + where;
			IDataReader reader;
			...
			try
			{
				...
				while (reader.Read())
				{
					...
					zap.adr = "ул. " + zap.ulica + ", д. " + zap.ndom;
		...
		public List<Town> GetTownList(Town finder, out Returns ret)
		{
			...
			using (IDbConnection connDb = GetConnection(connectionString))
			{
				...
				command.CommandText = string.Format(@"select 
					...
					where t.nzp_stat = {2}", pref, tableDelimiter, IsPg() ? ":nzp_stat" : "?");
		...
		public List<Rajon> GetRajonList(Rajon finder, out Returns ret)
		{
			...
			using (IDbConnection connDb = GetConnection(connectionString))
			{
				...
				from {0}_data{1}s_rajon t
					where t.nzp_town ={2}", pref, tableDelimiter, IsPg() ? ":nzp_town" : "?");

 ...
 " Set cur_val = cur_val + 1 " +
        ...
        public void GetSeriesWithoutSeries(IDbConnection connection, IDbTransaction trans, string pref, Series series, out Returns ret)
        {
            ...
            try
            {
                ...
                {
#if PG
                    if (kod == Series.Types.Area.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += " nextval('" + Points.Pref + "_data.s_area_nzp_area_seq') as nzp_area ";
                    }
                    else if (kod == Series.Types.Counter.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += " nextval('" + Points.Pref + "_data.counters_spis_nzp_counter_seq') as nzp_counter ";
                    }
                    else if (kod == Series.Types.Dom.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += " nextval('" + Points.Pref + "_data.dom_nzp_dom_seq') as nzp_dom ";
                    }
                    else if (kod == Series.Types.Geu.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += " nextval('" + Points.Pref + "_data.s_geu_nzp_geu_seq') as nzp_geu ";
                    }
                    else if (kod == Series.Types.Kvar.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += " nextval('"+Points.Pref+"_data.kvar_nzp_kvar_seq') as nzp_kvar ";
                    }
                    else if (kod == Series.Types.Payer.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += " nextval('" + Points.Pref + "_kernel.s_payer_nzp_payer_seq') as nzp_payer ";
                    }
                    else if (kod == Series.Types.Supplier.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields +=  " nextval('" + Points.Pref + "_kernel.supplier_nzp_supp_seq') as nzp_supp ";
                    }
                    else if (kod == Series.Types.Ulica.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields +=  " nextval('" + Points.Pref + "_data.s_ulica_nzp_ul_seq') as nzp_ul ";
                    }
#else
                    if (kod == Series.Types.Area.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_data:s_area_nzp_area_seq.nextval as nzp_area ";
                    }
                    else if (kod == Series.Types.Counter.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_data:counters_spis_nzp_counter_seq.nextval as nzp_counter ";
                    }
                    else if (kod == Series.Types.Dom.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_data:dom_nzp_dom_seq.nextval as nzp_dom ";
                    }
                    else if (kod == Series.Types.Geu.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_data:s_geu_nzp_geu_seq.nextval as nzp_geu ";
                    }
                    else if (kod == Series.Types.Kvar.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_data:kvar_nzp_kvar_seq.nextval as nzp_kvar ";
                    }
                    else if (kod == Series.Types.Payer.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_kernel:s_payer_nzp_payer_seq.nextval as nzp_payer ";
                    }
                    else if (kod == Series.Types.Supplier.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_kernel:supplier_nzp_supp_seq.nextval as nzp_supp ";
                    }
                    else if (kod == Series.Types.Ulica.GetHashCode())
                    {
                        if (fields != "") fields += ",";
                        fields += Points.Pref + "_data:s_ulica_nzp_ul_seq.nextval as nzp_ul ";
                    }
#endif
                }
                sql += fields;
#if PG
#else
                sql += " from " + Points.Pref + "_data:dual";
#endif
                ret = ExecRead(connection, trans, out reader, sql, true);
                if (!ret.result) return;
                ...
                if (reader.Read())
                {
                    foreach (int kod in list)
                    {
                        _Series val = series.GetSeries(kod);

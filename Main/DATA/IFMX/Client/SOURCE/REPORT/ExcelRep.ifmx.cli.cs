...
namespace STCLINE.KP50.DataBase
{
    ...
    public class ExcelRepClient : DataBaseHead
    {
        public List<ExcelUtility> GetListReport(ExcelUtility finder, out Returns ret)
        {
            ...
            try
            {
                if (!TableInWebCashe(conn_web, "excel_utility"))
                {
                ...
                if (finder.webLogin != "")
                {
                    sql = "select nzp_user from users where login = " + Utils.EStrNull(finder.webLogin);
                ...
                sql = "select count(*) from excel_utility " + " where (is_shared=1 " + where + " )  or (nzp_user = " + finder.nzp_user + where + ") ";
                ...
                sql = "select "  +
                    ...
                    " from excel_utility " +
                    " where (is_shared=1 " + where + " )  or (nzp_user = " + finder.nzp_user + where + ") " +
        ...
        public Returns AddMyFile(ExcelUtility finder)
        {
            ...
            try
            {
                ...
                sql.Append("insert into excel_utility (nzp_user, stats, prms, dat_in, rep_name, exc_comment, dat_today, exc_path, is_shared, file_name) ");
                ...
                {
                    ExecSQL(conn_db, "update excel_utility set dat_start = dat_in, dat_out = dat_in where nzp_exc = " + id, true);
                }
        ...
        public Returns SetMyFileProgress(ExcelUtility finder)
        {
            ...
            string sql = "update excel_utility set progress = " + finder.progress.ToString("N4").Replace(',','.') + " where nzp_exc = " + finder.nzp_exc;

...
{
    public class InstalmentFaktura : BaseFactura2
    {
        private const int MaxRevalNumber = 9;
        private const int MaxCountersCount = 8;
        private const int MaxSupplierCount = 9;
        private const int MaxInstalmentCount = 9;
        private const int MaxServCount = 20;

        public override string Name
        {
            get { return "Счет с рассрочкой по 354 пост. "; }
        }

        public override string Code { get { return "121"; } }

        public override string FileName { get { return "std354.frx"; } }
        ...
        protected override bool FillAdr()
        {
            ...
            Dr["adres"] = index + city + district + street + " д. " + NumberDom + flat;
        ...
        /// Заполнение таблицы по поставщикам
        /// </summary>
        protected void FillSumInstalment()
        {
            Instalment.GetInstalmentSum(Pref, NzpKvar);
            
            if (!Instalment.Allow) return;

            int index = 1;
            foreach(Instalment354 t in Instalment.Instalments)
            {
                if (index < MaxInstalmentCount)
                {
                    Dr["ins_serv" + index] = t.ServiceName;
                    Dr["ins_plat" + index] = t.SumPlat.ToString("0.00");
                    Dr["ins_prevplat" + index] = t.SumPreviosPlat.ToString("0.00");
                    Dr["ins_percsum" + index] = t.SumPercent().ToString("0.00");
                    Dr["ins_perc" + index] = t.Percent.ToString("0.00");
                    Dr["ins_charge" + index] = t.SumCharge().ToString("0.00");
                }
        ...
        public override DataTable MakeTable()
        {
            ...
            for (int i = 1; i < MaxInstalmentCount; i++)
            {
                table.Columns.Add("ins_serv" + i, typeof(string));
                table.Columns.Add("ins_plat" + i, typeof(string));
                table.Columns.Add("ins_prevplat" + i, typeof(string));
                table.Columns.Add("ins_percsum" + i, typeof(string));
                table.Columns.Add("ins_perc" + i, typeof(string));
                table.Columns.Add("ins_charge" + i, typeof(string));
            }

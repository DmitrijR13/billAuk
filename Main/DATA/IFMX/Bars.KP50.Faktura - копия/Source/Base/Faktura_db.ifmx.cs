...
namespace Bars.KP50.Faktura.Source.Base
{
    ...
    //----------------------------------------------------------------------
    {
        ...
        private bool GetNachSamara(string tableCharge, int nzpKvar, ref BaseFactura bill)
        {
            ...
            string moneyTable = "bill01_charge_" + (_finder.year_ - 2000).ToString("00") + ".fn_supplier" +
                                (_finder.month_).ToString("00");

            string dateTo = (_finder.year_).ToString() + "-" + (_finder.month_).ToString("00") + "-16";

            string numLs = bill.NumLs;
            ...
            string sql = " SELECT s.ordering, s.service_name as service, m.measure, su.name_supp as name_supp, a.tarif," +
                         ...
                         "         sum(a.sum_money) as sum_money, sum(coalesce(fs.sum_prih, 0)) as sum_money1, sum(a.rsum_tarif - a.gsum_tarif) as reval_gil, " +
                         "         sum(a.sum_insaldo) as sum_insaldo,  max(a.c_reval) as c_reval,                " +
                         "         sum(a.sum_nedop) as sum_nedop,  sum(a.sum_outsaldo) as sum_outsaldo           " +
                         " FROM  " + tableCharge + " a LEFT JOIN  (SELECT nzp_serv, sum(sum_prih) as sum_prih from " + moneyTable + " where num_ls = " + numLs + " and dat_prih < '" + dateTo + "' group by 1) fs on (fs.nzp_serv = a.nzp_serv) " +
                         " , " +
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    serv.Serv.MoneyTo = Decimal.Parse(reader["sum_money1"].ToString());
        ...
        private bool GetNachSamaraSupp(string tableCharge, int nzpKvar, ref BaseFactura bill)
        {
            ...
            sql.Append("         sum(a.sum_money) as sum_money, sum(coalesce(fs.sum_prih, 0)) as sum_money1, sum(a.rsum_tarif - a.gsum_tarif) as reval_gil, ");
            ...
            sql.Append(" FROM  " + tableCharge + " a LEFT JOIN  (SELECT nzp_serv, sum(sum_prih) as sum_prih from " + moneyTable + " where num_ls = " + numLs + " and dat_prih < '" + dateTo + "' group by 1) fs on (fs.nzp_serv = a.nzp_serv) ");
            sql.Append(" ,  ");
        ...
        private bool GetNachGubkin(string tableCharge, int nzpKvar, ref BaseFactura bill)
        {
            ...
            sql.Append("         sum(a.sum_charge) as sum_charge, sum(a.sum_money) as sum_money, sum(coalesce(fs.sum_prih, 0)) as sum_money1, ");
        ...
        private bool GetNachSaha(string tableCharge, int nzpKvar, ref BaseFactura bill)
        {
            ...
            sql.Append("         sum(a.real_charge) as real_charge, sum(a.sum_outsaldo) as sum_outsaldo, ");
            sql.Append("         sum(a.sum_charge) as sum_charge, sum(a.sum_money) as sum_money, sum(coalesce(fs.sum_prih, 0)) as sum_money1, sum(0) as reval_gil,");
        ...
        private bool GetNachTula(string tableCharge, int nzpKvar, ref BaseFactura bill)
        {
            ...
            sql = " SELECT s.ordering, s.service_name as service, m.measure, su.payer as name_supp,su.inn, b.bank_name, b.rcount," +
                         ...
                         "        sum(a.rsum_tarif - a.gsum_tarif + a.reval - a.sum_nedop) as reval, sum(0) as sum_sn, " +
                         "        sum(a.real_charge) as real_charge, sum(a.sum_outsaldo) as sum_outsaldo, " +
                         "        sum(a.sum_charge) as sum_charge, sum(a.sum_money) as sum_money, sum(coalesce(fs.sum_prih, 0)) as sum_money1, sum(a.rsum_tarif-a.gsum_tarif) as reval_gil, " +
                         "        sum(a.sum_insaldo) as sum_insaldo, max(a.c_calc) as c_calc, max(a.c_reval) as c_reval " +
                         " FROM  " +
                         Points.Pref + DBManager.sKernelAliasRest + "services s, " +
                         Points.Pref + DBManager.sKernelAliasRest + "s_measure m, " +
                         "        " + tableCharge + " a LEFT JOIN  (SELECT nzp_serv, sum(sum_prih) as sum_prih from " + moneyTable + " where num_ls = " + numLs + " and dat_prih < '" 
                         + dateTo + "' group by 1) fs on (fs.nzp_serv = a.nzp_serv) " +
                         " , " +
                         "        " + Points.Pref + DBManager.sKernelAliasRest + "supplier sup, " +
        ...
        private bool GetNachKaluga(string tableCharge, int nzpKvar, ref BaseFactura bill)
        {
            ...
            string sql = " SELECT s.ordering, s.service_name as service, m.measure, su.payer as name_supp,su.inn," +
                ...
                //" AND date_rcl = '" + new DateTime(bill.Year, bill.Month, 1).ToShortDateString() + "'), " +
                         " AND month_ = " + bill.Month + ") LEFT JOIN  (SELECT nzp_serv, sum(sum_prih) as sum_prih from " + moneyTable + " where num_ls = " + numLs + " and dat_prih < '"
                         + dateTo + "' group by 1) fs on (fs.nzp_serv = a.nzp_serv) " +
                         " , " +
                         "        " + Points.Pref + DBManager.sKernelAliasRest + "supplier sup " +
                         "        left join  " + Points.Pref + DBManager.sKernelAliasRest + "s_payer su " +
        ...
        private void PrepareGroupTempTable(FakturaBlockTable fBlock, out Returns ret)
        {
            ...
            var sql = new StringBuilder();
            string tXxSpls;
            int dayInMonth = DateTime.DaysInMonth(_finder.year_, _finder.month_);
            //string datCalc = "01." + _finder.month_ + "." + _finder.year_;
            string datCalcS = dayInMonth + "." + _finder.month_ + "." + _finder.year_;
            string datCalc = "01." + _finder.month_ + "." + _finder.year_;
            try
            {
                ret = DBManager.OpenDb(connWeb, true);
            ...
            while (goodReader.Read())
            {
                ...
                sql.Append("                AND a.dat_s<='" + datCalcS + "'");
                ...
                sql.Append("               AND p.dat_s<='" + datCalcS + "'");
                        ...
                        try
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                if (reader["cnt_stage"].ToString() == "1" || (reader["cnt_stage"].ToString() == "5" && servVolume.NzpServ == 8)
                                    || (reader["cnt_stage"].ToString() == "0" && servVolume.NzpServ == 9)

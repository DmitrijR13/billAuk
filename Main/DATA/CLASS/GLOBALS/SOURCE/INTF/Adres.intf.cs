...
namespace STCLINE.KP50.Interfaces
{
    ...
    */
    ...
    public interface IAddressRemoteObject : I_Adres, IDisposable { }
    ...
    public interface I_Adres
    {
        ...
        [OperationContract]
        List<Dom> GetDom2(Dom finder, Service servfinder, out Returns ret);

        [OperationContract]
        List<Ulica> GetUlica(Dom finder, enSrvOper srv, out Returns ret);

        [OperationContract]
        List<Town> GetTownList(Town finder, out Returns ret);

        [OperationContract]
        List<Rajon> GetRajonList(Rajon finder, out Returns ret);
        ...
        [OperationContract]
        Returns SaveAreaPayer(Area finder);

        [OperationContract]
        Returns DeleteArea(Area finder);
        ...
        [OperationContract]
        List<Finder> GetPointsDom(Finder finder, out Returns ret);
        ...
        [OperationContract]
        List<Area_ls> LoadCurrentLsSupplier(Area_ls finder, out Returns ret);

        [OperationContract]
        void DeleteSupplierLs(Area_ls finder, out Returns ret);

        [OperationContract]
        Area_ls LoadCurrentAliasLs(Area_ls finder, out Returns ret);

        [OperationContract]
        void SaveSupplierLs(Area_ls finder, out Returns ret);
        ...
        [OperationContract]
        Returns DeleteGroup(Group finder);
        ...
        [OperationContract]
        List<Ls> LoadLsData(Ls finder, out Returns ret);

        [OperationContract]
        List<KvarPkodes> GetPkodes(Ls finder, out Returns ret);
        ...
        [OperationContract]
        List<RajonDom> LoadRajonDom(RajonDom finder, out Returns ret);
        ...
        [OperationContract]
        Returns ChangeAddressLs(Finder finder);

        [OperationContract]
        Returns DeleteLs(Ls finder);
         [OperationContract]
        bool SaveListGroupLSBySelectedDoms(Group finder, out Returns ret);

        [OperationContract]
         Returns GetCountLSBySelectedDom(Group finder);

        /// <summary>
        /// Перезаписывает tXX_spls по списку выбранных должников
        /// </summary>
        /// <param name="finder"></param>
        /// <returns></returns>
        [OperationContract]
        Returns FindLsFromDeptorSpis(Ls finder);
    ...
    [Serializable]
    [DataContract]
    public class RajonDom 
    {
        [DataMember]
        public int nzp_raj_dom { get; set; }
        [DataMember]
        public string rajon_dom { get; set; }
        [DataMember]
        public string alt_rajon_dom { get; set; }
    ...
    public class Ulica : Rajon
    {
        ...
        public string ulica_full
        {
            get
            {
                return ulica_short.Trim() + (rajon.Trim() == "" ? "" : " / " + (rajon.Trim() ?? "")) + (town.Trim() == "" ? "" : " / " + (town.Trim() ?? ""));
            }
            set { }
        }
        ...
        public string ulica_short_for_dd
        {
            get
            {
                return String.IsNullOrEmpty(ul_short_for_dd) ? ulica_short : ul_short_for_dd;
            }
            set
            {
                ul_short_for_dd = value;
            }
        }
        public Ulica()
            : base()
    ...
    public class Ls : Dom
    {
        ...
        public void CopyAttributes(Ls newLs)
        {
            ...
            this.CopyTo(newLs);
            newLs.nzp_kvar = nzp_kvar;
            newLs.num_ls = num_ls;
            newLs.num_ls_s = num_ls_s;
            newLs.num_ls_litera = num_ls_litera;
            newLs.pkod = pkod;
            newLs.typek = typek;
            newLs.stateID = stateID;
            newLs.moving = moving;
            newLs.stateValidOn = stateValidOn;
            newLs.fio = fio;
            newLs.nkvar = nkvar;
            newLs.nkvar_n = nkvar_n;
            newLs.checkstateIncalcMonth = checkstateIncalcMonth;
            newLs.uch = uch;
            newLs.porch = porch;
            newLs.phone = phone;
            newLs.nkvar_po = nkvar_po;
            newLs.remark = remark;
            newLs.chekexistls = chekexistls;
            newLs.is_pasportist = is_pasportist;
            newLs.mark = mark;

            newLs.gil_kol = gil_kol;

            newLs.dopParams = dopParams;
            newLs.dopUsl = dopUsl;
            newLs.dat_calc = dat_calc;

            newLs.copy_ls = copy_ls;
            newLs.copy_ls_from = copy_ls_from;
            newLs.gen_pu = gen_pu;
            newLs.kvar_s = kvar_s;
            newLs.kvar_po = kvar_po;
            newLs.withUk = withUk;
            newLs.withGeu = withGeu;
            newLs.withUchastok = withUchastok;
        }
    }
    ...
    public class KvarPkodes : Finder
    {
        ...
        public KvarPkodes()
            : base()
        {
            id = nzp_kvar = nzp_payer = is_princip = area_code = is_default = 0;
            pkod = pkod10 = pkod_text = "";
        }
    }
    ...
    public class MapObject
    {
        ...
        /// <summary>
        /// тип сущности, к которой привязан объект на карте: -1 - ключ для карты, -2 - координаты местности по умолчанию, 1 - дом, 2 - улица, 3 - Управляющая организация
        /// </summary>
        [DataMember]
    ...
    [DataContract]
    public class House_kodes : Finder
    //----------------------------------------------------------------------
    {
        [DataMember]
        public int id { get; set; }
        ...
        public House_kodes() : base()
        {
            id = nzp_dom = nzp_payer = num = 0;
            kod_dom = payer = comment = dat_when = user = "";
        }
    }
    ...
    //----------------------------------------------------------------------
    {
        ...
        public DateTime RecordDateTime
        {
            get
            {
                if (month_ > 0 && year_ > 0)
                {
                    return new DateTime(year_, month_, 1);
                }
                else
    ...
    /// <summary>
    /// Параметры для разделения ЛС
    /// </summary>
    [DataContract]
    public class SplitLsParams : Finder
    {
        /// <summary>
        /// номер лс по порядку
        /// </summary>
        ...
        /// </summary>
        [DataMember]
        public string dat_open { get; set; }

        /// <summary>
        /// общая площадь
        /// </summary>
        [DataMember]
        public decimal tot_squ { get; set; }

        /// <summary>
        /// жилая площадь
        /// </summary>
        [DataMember]
        public decimal liv_squ { get; set; }
        ...
        /// </summary>
        [DataMember]
        public decimal otopl_squ { get; set; }
        ...
        /// </summary>
        [DataMember]
        public int nkvar_n { get; set; }

        /// <summary>
        /// разделять сальдо
        /// </summary>
        [DataMember]
        public bool is_split_saldo { get; set; }

        public SplitLsParams()
            : base()
        {
            num = 0;
            nzp_kvar = 0;

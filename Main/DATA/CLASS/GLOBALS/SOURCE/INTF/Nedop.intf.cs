...
namespace STCLINE.KP50.Interfaces
{
    ...
    public interface INedopRemoteObject : I_Nedop, IDisposable { }
    ...
    //----------------------------------------------------------------------
    {
        ...
        public void CopyTo(Nedop item)
        {
            if (item == null) return;
            base.CopyTo(item);
            item._dat_s = this._dat_s;
            item._dat_po = this._dat_po;
            item._dat_when = this._dat_when;

            item.nzp_nedop = this.nzp_nedop;
            item.nzp_serv = this.nzp_serv;
            item.service = this.service;
            item.nzp_supp = this.nzp_supp;
            item.supplier = this.supplier;
            item.tn = this.tn;
            item.comment = this.comment;
            item.is_actual = this.is_actual;
            //item.remark = this.remark;
            item.nzp_user_when = this.nzp_user_when;
            item.user_name = this.user_name;
            item.nzp_kind = this.nzp_kind;
            item.kind = this.kind;
            item.block = this.block;
            item.duration = this.duration;
            item.cnt_ls = this.cnt_ls;
        }

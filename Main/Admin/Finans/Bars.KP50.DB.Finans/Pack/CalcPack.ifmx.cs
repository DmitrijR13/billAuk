...
namespace STCLINE.KP50.DataBase     
{
    ...
    public class DbCalcPack : DataBaseHead
    {
        ...
        bool isUseSupplier = false; // Признак использования новой схемы учёта поступелений по договорам
        ...
        #endif
        string strKodSumForCharge_MM="23,33,41,40,50";  // Ежемесячные квитанции 
        ...
        public void MessageInPackLog(IDbConnection conn_db, CalcTypes.PackXX packXX, string msg, bool err, out Returns ret)
        {
        ...
        //-----------------------------------------------------------------------------
        void CreatePackLog(IDbConnection conn_db2, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
...
ExecSQL(conn_db, " update statistics for table " + packXX.pack_log_tab, true);
        ...
        //распределение пачки
        public bool CalcPackXX(CalcTypes.ParamCalc paramcalc, bool isManualDistribution, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
            ...
            bool result = CalcPackXX(conn_db, paramcalc, isManualDistribution, out ret, setTaskProgress);
        ...
        {
            return CalcPackXX(conn_db, paramcalc, isManualDistribution, out ret, null);
        }

        public bool CalcPackXX(IDbConnection conn_db, CalcTypes.ParamCalc paramcalc, bool isManualDistribution, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        //-----------------------------------------------------------------------------
        {
            return CalcPackXX(conn_db, paramcalc, 0, true, isManualDistribution, out ret, setTaskProgress);
        }


        //-----------------------------------------------------------------------------
        bool CreateSelKvar(IDbConnection conn_db, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            return CreateSelKvar(conn_db, packXX, false, out ret);
        }
        //-----------------------------------------------------------------------------
        bool CreateSelKvar(IDbConnection conn_db, CalcTypes.PackXX packXX, bool cur_dat_uchet, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            ret = Utils.InitReturns();
          ...
          if (isDebug)
          {
            ...
            #else
                sqlText =
                    " Insert into t_selkvar (nzp_kvar, num_ls, nzp_wp, nzp_dom, nzp_area, nzp_geu, nzp_pack, nzp_pack_ls, kod_sum, dat_uchet, dat_month, dat_vvod, g_sum_ls, id_bill, nzp_supp, pkod)  " +
                    " Select k.nzp_kvar, k.num_ls, " + packXX .paramcalc.nzp_wp+ ", k.nzp_dom, k.nzp_area, k.nzp_geu, p.nzp_pack, p.nzp_pack_ls, p.kod_sum, p.dat_uchet, p.dat_month, p.dat_vvod, p.g_sum_ls, p.id_bill,p.nzp_supp, p.pkod  " +
                    " From " + packXX.paramcalc.pref + "_data:kvar k, " + packXX.pack_ls + " p " +
                    " Where k.num_ls = p.num_ls " +
                        s_dat_uchet + //фильтрация по dat_uchet
                    "   and k." + packXX.paramcalc.where_z +
           ...
           if ((packXX.nzp_pack>0)&(pack_type ==20))
                    sqlText =sqlText +" and p.kod_sum in (40,50) ";
            ret = ExecSQL(conn_db, sqlText , true);
            if (!ret.result)
            {
                DropTempTablesPack(conn_db);
        ...
        {
            return CalcPackXX(conn_db, paramcalc, nzp_pack_ls, checkPackDistrb, isManualDistribution, out ret, null);
        }

        public bool CalcPackXX(IDbConnection conn_db, CalcTypes.ParamCalc paramcalc, int nzp_pack_ls, bool checkPackDistrb, bool isManualDistribution, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
                    ...
                    */ 
                    ...
                    string tableGilsums = Points.Pref + "_fin_" + (paramcalc.calc_yy - 2000).ToString("00") + paramcalc.ol_srv + tableDelimiter + "gil_sums a";
                        ...
                        if (pack_type == 20)  // Чужие средства
                        {
                            ...
                            " Select  a.num_ls, a.nzp_pack_ls, a.nzp_serv, a.nzp_supp, a.sum_oplat," + kod_sum + ",a.dat_month, cast ('" + dat_vvod + "' as DATE) , CAST( " + packXX.paramcalc.dat_oper + " as DATE) " +
                            " From  " + tableGilsums +
                        ...
                        else
                        {
                            ...
                            " Select distinct  a.num_ls, a.nzp_pack_ls, a.nzp_serv, a.nzp_supp, a.sum_oplat," + kod_sum + ",a.dat_month, cast('" + dat_vvod + "' as date), cast(" + packXX.paramcalc.dat_oper + " as date), 0, a.sum_oplat, 0 " +
                            " From  " + tableGilsums +
                            " Where  a.nzp_pack_ls = " + packXX.nzp_pack_ls.ToString() + " and sum_oplat <>0 ";
                        }
            ...
            {   // ---------------- Если автоматическое распределение оплаты согласно заложенным алгоритмам ---------------------------------------------------------
                ...
                sqlText="update t_selkvar set is_open = -1 where  "+
                    ...
                    " Where k.nzp = t_selkvar.nzp_kvar  AND k.nzp_prm = 51 and k.val_prm = '3') ";
...
/*
                ...
                sqlText =
                    ...
                    " where t_charge.num_ls = a.num_ls and t_charge.nzp_serv = a.nzp_serv and t_charge.nzp_supp = a.nzp_supp ) where  nzp_pack_ls not in (select nzp_pack_ls from t_selkvar where kod_sum in (40, 50))  ";
                ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();
                if (!ret.result)
                {
                    DropTempTablesPack(conn_db);
                ...
                */
                ...
                sqlText =
                    " update t_opl set kod_info = 0 where kod_info >0 and nzp_pack_ls in (select i.nzp_pack_ls from t_iopl i where i.sum_prih <> i.g_sum_ls) and kod_info not in (110,177) ";
                ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();
                if (!ret.result)
                {
                    DropTempTablesPack(conn_db);
                ...
                sqlText =
                    " update t_opl set kod_info = 0 where kod_info >0 and kod_info not in (110,177) and nzp_pack_ls not in (select i.nzp_pack_ls from t_iopl i )  ";
                ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();
                if (!ret.result)
                {
                    DropTempTablesPack(conn_db);
        ...
        //-----------------------------------------------------------------------------
        bool OplSamara(IDbConnection conn_db, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            DataTable dt;
            DataRow row;
...
*/
            ...
            sqlText =
                       ...
                       " where  g_sum_ls_ost > 0 and kod_sum in (" + strKodSumForCharge_MM + ",81) and kod_info in (103,106)  and isum_charge>0";
#else
            sqlText =
                    " Update t_opl " +
                    " Set  " +
        ...
        //-----------------------------------------------------------------------------
        bool StandartReparation(IDbConnection conn_db, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            ...
            if (isDistributionForInSaldo != true)
            {
                ...
                if (Points.packDistributionParameters.EnableLog & packXX.nzp_pack_ls > 0)
                {
                    ...
                    { s1 = s1 + "'Положительная часть исходящего сальдо'"; }
                        ...
                        if (Points.packDistributionParameters.distributionMethod == PackDistributionParameters.PaymentDistributionMethods.CurrentMonthSumInsaldo)
                        { s1 = s1 + "'Исходящеее сальдо'"; }
                            ...
                            { s1 = s1 + "'Положительная часть начислено к оплате'"; }
                                ...
                                if (Points.packDistributionParameters.distributionMethod == PackDistributionParameters.PaymentDistributionMethods.LastMonthSumCharge)
...
//#endif
            sqlText = "select nzp_pack_ls, nzp_serv , sum_oplat, is_union,nzp_supp, nzp_sums from " + Points.Pref + "_fin_" + (Convert.ToDateTime(Points.DateOper).Year - 2000).ToString("00") + tableDelimiter+"gil_sums a where a.nzp_serv>0  and  a.nzp_pack_ls in (select a.nzp_pack_ls from  t_opl a ) ";
                ...
                if (rowGilSums["nzp_pack_ls"] != DBNull.Value) 
                {
                    ...
                    if (nzp_supp != "0")
                   {
                       ...
                       ExecSQL(conn_db, " insert into t_gil_sums (nzp_pack_ls, nzp_serv, sum_prih,sum_prih_u, nzp_supp) select nzp_pack_ls, nzp_serv, sum_oplat, sum_oplat,nzp_supp from " + Points.Pref + "_fin_" + (Convert.ToDateTime(Points.DateOper).Year - 2000).ToString("00") +
                           tableDelimiter + "gil_sums a where a.nzp_sums = " + Convert.ToString(rowGilSums["nzp_sums"]), true);
                   } else
                    {
                        #if PG
                            sqlText =
            ...
            sqlText = " update t_opl set  sum_prih_u=sum_prih  where nzp_pack_ls in (select b.nzp_pack_ls from t_gil_sums b) and sum_prih_u =0";
            ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();
            
            sqlText = " update t_gil_sums set  sum_prih_u=sum_prih  where kod_info=114 and sum_prih_u =0";
            ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();

            sqlText = " update t_opl set  kod_info = 114 where   nzp_pack_ls in (select b.nzp_pack_ls from t_gil_sums b) ";
            ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();
            ...
            */
            ...
            sqlText = " update t_opl set   g_sum_ls_ost = (select a.g_sum_ls_ost from t_ostatok a where a.nzp_pack_ls = t_opl.nzp_pack_ls) where nzp_pack_ls in (select b.nzp_pack_ls from t_gil_sums b)  and sum_prih_u=0 ";
            ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();

            sqlText = " update t_opl set  kod_info = 114 where sum_prih_u >0  ";
            ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();

            sqlText = "update t_opl set irsum_tarif = (select sum(rsum_tarif) from t_charge t where t_opl.nzp_pack_ls  = t.nzp_pack_ls and t.nzp_key not in (select a.nzp_key from t_opl a "+
                "where a.nzp_pack_ls = t_opl.nzp_pack_ls and a.sum_prih_u >0) )  where   t_opl.nzp_pack_ls in (select b.nzp_pack_ls from t_gil_sums b) "; 
            ...
            sqlText =
                        ...
                        " where  g_sum_ls_ost > 0 and kod_info in (114, 0) and  irsum_tarif<>0 and   nzp_pack_ls in (select b.nzp_pack_ls from t_gil_sums b) and sum_prih_u=0 ";
...
*/
            ...
            sqlText =
                    ...
                    " Set kod_info = 101, " +
                    "     sum_prih_d = sum_charge" + // Погашение долга
                    " where isum_charge = g_sum_ls-gil_sums and kod_sum in (" + strKodSumForCharge_MM + ",81) and kod_info = 0  and  g_sum_ls-gil_sums>0 ";
            ...
            */
            ...
            if ( // Способ начисления sum_charge по методу C,D,E,F
                  (Points.packDistributionParameters.distributionMethod  == PackDistributionParameters.PaymentDistributionMethods.CurrentMonthSumInsaldo) ||
                  (Points.packDistributionParameters.distributionMethod == PackDistributionParameters.PaymentDistributionMethods.LastMonthSumCharge)                   
                )
            {
               // Если по всем услугам отрицательные суммы в sum_charge (по лицевому счёту всё погашено) 
                sqlText = " update t_opl set kod_info = 104 where (select sum(b.sum_outsaldo) from t_charge b where t_opl.nzp_kvar = b.nzp_kvar and b.sum_outsaldo<0  ) = isum_outsaldo and kod_info = 103 ";
            ...
            // 3.4.6 Определить остаток от погашения
                ...
                sqlText =
                            ...
                            " Set  " +
                            "     sum_prih_a = g_sum_ls_ost * (rsum_tarif/irsum_tarif)" +
                            " where  g_sum_ls_ost > 0 and kod_sum in (" + strKodSumForCharge_MM + ",81) and kod_info in (103,104,114) and g_sum_ls_ost>0 and isdel=0  and irsum_tarif>0";                    
                ...
                sqlText =
                            ...
                            " where  g_sum_ls_ost > 0 and kod_sum in (" + strKodSumForCharge_MM + ",81) and kod_info in (103,104,114) and g_sum_ls_ost>0 and isdel=0  and isum_outsaldo>0";
                ...
                sqlText = " Update t_opl " +
                                        " SET  g_sum_ls_ost = (select g_sum_ls_ost from t_ostatok where t_ostatok.nzp_pack_ls = t_opl.nzp_pack_ls)" +
                                        " where  kod_sum in (" + strKodSumForCharge_MM + ",81) and kod_info in (103,104,114)  and isdel=0  and sum_insaldo>0";
                ret = ClassDBUtils.ExecSQL(sqlText, conn_db, ClassDBUtils.ExecMode.Log).GetReturnsType().GetReturns();
                if (!ret.result)
                {
                    DropTempTablesPack(conn_db);
                ...
                sqlText =
                            ...
                            " where  g_sum_ls_ost > 0 and kod_sum in (" + strKodSumForCharge_MM + ",81) and kod_info in (103,104,114) and g_sum_ls_ost>0 and isdel=0  and isum_insaldo>0";
                ...
                if (Points.packDistributionParameters.EnableLog & count > 0 & packXX.nzp_pack_ls > 0)
                {
                    ...
                    row = dt.Rows[0];
                    s1 = Convert.ToString(row["sum_"]);
                    MessageInPackLog(conn_db, packXX, "Распределение аванса " + s1 + " руб по 'Входящему сальдо (+)'", false, out r);
                }
        ...
        //-----------------------------------------------------------------------------
        bool StandardActiveAndInactiveSuppliers(IDbConnection conn_db, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
        ...
        //-----------------------------------------------------------------------------
        bool StandardSpecial(IDbConnection conn_db, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
        ...
        public bool CalcPackDel(CalcTypes.ParamCalc paramcalc, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
            string conn_kernel = Points.GetConnByPref(paramcalc.pref);
            IDbConnection conn_db = GetConnection(conn_kernel);
            ret = OpenDb(conn_db, true);
        ...
        public bool CalcPackDel(IDbConnection conn_db, CalcTypes.ParamCalc paramcalc, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
            ret = Utils.InitReturns();
            Returns r = Utils.InitReturns();
        ...
        public void CalcDistribFon(DateTime dat_s, DateTime dat_po, int nzp_user, out Returns ret)
        {
            ...
            calcfon.nzp = Convert.ToInt32(dat_s.ToString("yyyyMMdd"));
            calcfon.nzpt = Convert.ToInt32(dat_po.ToString("yyyyMMdd"));
            calcfon.txt = "Учет к перечислению за период с " + dat_s.ToShortDateString();
        ...
        /// <param name="ret"></param>
        /// <returns></returns>
        public bool CalcPackOt(CalcTypes.ParamCalc paramcalc, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
            string conn_kernel = Points.GetConnByPref(paramcalc.pref);
            IDbConnection conn_db = GetConnection(conn_kernel);
            ret = OpenDb(conn_db, true);
        ...
        public bool CalcPackOt(IDbConnection conn_db, CalcTypes.ParamCalc paramcalc, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
            return CalcPackOt(conn_db, paramcalc, 0, out ret, setTaskProgress);
        }

        public bool CalcPackOt(IDbConnection conn_db, CalcTypes.ParamCalc paramcalc, int nzp_pack_ls, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        {
           ...
           //bool b=FillSumOplForNebo(conn_db, 1, "04.10.2013", "04.10.2013", out ret);
            ...
            if (paramcalc.pref.Trim() == "")
            {
                    ...
                    " Where  nzp_pack = " + packXX.nzp_pack + " and coalesce(num_ls,0)=0  and dat_uchet = '" + Points.DateOper.ToShortDateString()+"'"+sConvToDate; 
#else
                sql_text =
                    " update  " + packXX.pack_ls + "  set inbasket = 0 , alg = 0, dat_uchet = null " +
                    " Where  nzp_pack = " + packXX.nzp_pack + " and NVL(num_ls,0)=0 and dat_uchet = '" + Points.DateOper.ToShortDateString() + "'" + sConvToDate;
                #endif
                ret = ExecSQL(conn_db,
                    sql_text
                    , true);
            ...
            //if (Constants.Trace) Utility.ClassLog.WriteLog("Старт Выборка оплат");
                ...
                sql_text =
                    ...
                    "   and  ( (p.dat_uchet ='" + Points.DateOper.ToShortDateString()+"'" + sConvToDate+") or (p.inbasket=1))  )" +
                    " union all " +
            ...
            if (count==0)
            {
                    ...
                    ret.text = "Отсутствуют оплаты для отмены распределения. Данная операция может быть произведена только над оплатами, распределёнными за " + Points.DateOper.ToShortDateString();
                    ret.result = false;
                    //Utility.ClassLog.WriteLog(ret.text);
            ...
            int i = Points.PointList.Count+2;
            int j = 0;
            foreach (_Point zap in Points.PointList)
            {
        ...
        public bool CalcPackLs2(int nzp_pack_ls, DateTime date, bool to_dis, bool is_manual, out Returns ret, int nzp_user)
        {
            ...
            if (nzp_kvar > 0)
            {
                ...
                {
                    b = CalcPackXX(conn_db, paramcalc, nzp_pack_ls, false, isManualDistribution, out ret, null); //распределение оплаты
        ...
        public void PackFonTasks(int nzp_pack, CalcFonTask.Types task, out Returns ret)
        {
        ...
        /// <param name="ret"></param>
        public void PackFonTasks(int nzp_pack, int nzp_user, CalcFonTask.Types task, out Returns ret)
        {                        
            IDbConnection conn_db = GetConnection(Constants.cons_Kernel);
            ret = OpenDb(conn_db, true);
            if (!ret.result)
        ...
        public void PackFonTasks(int nzp_pack, int nzp_user, CalcFonTask.Types task, out Returns ret, IDbConnection conn_db, IDbTransaction transaction)
        {
...
" Update " + pack +
        ...
        //-----------------------------------------------------------------------------
        void LoadLocalPaXX(IDbConnection conn_db, CalcTypes.PackXX packXX, bool load, out Returns ret)
        //-----------------------------------------------------------------------------
        {
        ...
        //-----------------------------------------------------------------------------
        void CreatePaXX(IDbConnection conn_db2, CalcTypes.PackXX packXX, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            CreatePaXX(conn_db2, packXX, true, out ret);
        }
        //-----------------------------------------------------------------------------
        void CreatePaXX(IDbConnection conn_db2, CalcTypes.PackXX packXX, bool delete, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            ret = Utils.InitReturns();
...
ret = ExecSQL(conn_db, " Database " + Points.Pref + "_kernel", true); //!!!!
        ...
        //-----------------------------------------------------------------------------
        public void DistribPaXX_1(DateTime dat_s, DateTime dat_po, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        //-----------------------------------------------------------------------------
        {
            ret = Utils.InitReturns();
            ...
            while (dat <= dat_po)
            {
                ...
                paramcalc.DateOper = dat;
                //CalcUderFndistrib(conn_db, dat,0, out  ret);
                ...
                //расчет сальдо
                DistribPaXX(conn_db, paramcalc, out ret, setTaskProgress,i, ref  j);
                if (!ret.result)
                {
                    conn_db.Close();
                    return;
...
ExecSQL(conn_db, " Database " + Points.Pref + "_kernel", true);
 ...
 " and nzp_dom in (select nzp_dom from " + fn_operday_dom_mc + ") ";
        ...
        //-----------------------------------------------------------------------------
        {
            ...
            MonitorLog.WriteLog("Начало расчёта сальдо перечисления за " + paramcalc.dat_oper, MonitorLog.typelog.Info, 1, 222, true);
            ...
            */
            ...
            ret = ExecSQL(conn_db, " Create " + sCrtTempTable + " table ttt_paxx (nzp_supp integer default 0 , nzp_payer integer, nzp_area integer, nzp_dom integer, nzp_serv integer, nzp_bank integer, kod integer,  sum_prih " + sDecimalType + "(14,2) default 0.00) " + sUnlogTempTable, true); 
            if (isUseSupplier)
            {
                sqlText = "insert into ttt_paxx ( nzp_supp, nzp_payer, nzp_area, nzp_dom, nzp_serv, nzp_bank,  kod,  sum_prih) " +
                    " Select a.nzp_supp, 0 as nzp_payer, a.nzp_area, a.nzp_dom, a.nzp_serv, a.nzp_bank,  1 as kod, sum(sum_prih) as sum_prih " +
                    " From " + fn_packXX.fn_pa_xx + " a,  t_dom d " +
            ...
            ExecSQL(conn_db, " Create  index ix1_ttt_paxx on ttt_paxx (nzp_payer, nzp_area, nzp_dom, nzp_serv, nzp_bank, nzp_supp) ", true);            
            ...
            ret = ExecSQL(conn_db,
                ...
                " Where 0 < ( Select count(*) From " + fn_packXX.fn_distrib + " a " +
                            " Where  "+                            
                            "    a.nzp_payer= ttt_paxx.nzp_payer " +
...
ExecSQL(conn_db, " Update statistics for table ttt_paxx ", true);
          ...
          #if PG
  ret = ExecSQL(conn_db,
                " Select nzp_supp,nzp_payer, nzp_area,nzp_dom,nzp_serv, -1 as nzp_bank, 1 as kod, sum(sum_send) as sum_send " +
                 " Into temp ttt_paxx  "+
  ...
  ret = ExecSQL(conn_db,
            ...
            ExecSQL(conn_db, " Create index ix1_ttt_paxx on ttt_paxx (nzp_payer, nzp_area, nzp_dom,nzp_serv, nzp_bank) ", true);
            #if PG
ExecSQL(conn_db, " analyze ttt_paxx ", true);
#else
ExecSQL(conn_db, " Update statistics for table ttt_paxx ", true);
 ...
 " Select max(1) as kod, sum(g_sum_ls) as sum_rasp From " + fn_packXX.pack + " p,"+ fn_packXX.pack_ls + " a, " + table_kvar + " b " +
        ...
        //--------------------------------------------------------------------------------
        void EqualPack(IDbConnection conn_db, CalcTypes.PackXX packXX, out Returns ret)
        //--------------------------------------------------------------------------------
        {
            ret = Utils.InitReturns();
...
sql = "update  " + baseFin + ":pack set flag = 11, sum_pack = (select sum(a.g_sum_ls) from " + baseFin + ":pack_ls a WHERE a.nzp_pack = " + baseFin + ":pack.nzp_pack), sum_rasp = 0, sum_nrasp = " + "(select sum(a.g_sum_ls) from " + baseFin + ":pack_ls a WHERE a.nzp_pack = " + baseFin + ":pack.nzp_pack), " +
                ...
                db1.PackFonTasks(nzp_pack_to, CalcFonTask.Types.DistributePack, out ret2);  // Отдаем пачку с переплатами на распределение
                db1.Close();
                if (!ret2.result)
                {
                    ret.result = false;
        ...
        // Сохранить пометку fn_operday_dom_mc
        public bool Update_reval(IDbConnection conn_db, IDbTransaction transactionID, CalcTypes.PackXX fn_packXX, bool flgUpdateTotal, out Returns ret)
        {
...
#warning Проверить на postgree
            ...
            ExecSQL(conn_db, transactionID, " Create  index ix1_ttt_paxx on ttt_paxx (nzp_payer, nzp_area, nzp_dom, nzp_serv, nzp_bank) ", true);
           #if PG
 ...
 //DataTable drr2 = ViewTbl(conn_db, " select * from ttt_paxx  ");
        ...
        //-----------------------------------------------------------------------------
        public void UpdateSaldoFndistrib(IDbConnection conn_db, IDbTransaction transaction, DateTime min_dat_oper, int nzp_payer, int nzp_area, out Returns ret)
        //-----------------------------------------------------------------------------
        {
            ret = Utils.InitReturns();
            ...
            while (dat <= lastDayDatCalc)
            {
                ...
                sql_text = " Drop table ttt_dis_prev ";
                ExecSQL(conn_db, transaction, sql_text, false);

                DateTime d = fn_packXX.paramcalc.DateOper.AddDays(-1);

                sql_text= " Create " + sCrtTempTable + " table ttt_dis_prev (nzp_payer integer, nzp_area integer, nzp_dom integer, nzp_serv integer, nzp_bank integer,  sum_in " + sDecimalType + "(14,2) default 0.00) " + sUnlogTempTable;
                ret = ExecSQL(conn_db, transaction, sql_text, true);
                if (!ret.result)
                {
                    return;
                }

                sql_text = " insert Into  ttt_dis_prev (nzp_payer, nzp_area, nzp_dom,nzp_serv, nzp_bank,  sum_in) " +
                    " Select a.nzp_payer, a.nzp_area, a.nzp_dom,a.nzp_serv, a.nzp_bank, sum(a.sum_out) as sum_in " +
                    " From " + fn_packXX.fn_distrib_prev + " a " +
                    " Where a.dat_oper = '" + d.ToShortDateString() + "' " + sConvToDate + " ";
                if (nzp_payer > 0)
                {
                    sql_text = sql_text + " and a.nzp_payer = " + nzp_payer;
                }
                if (nzp_area > 0)
                {
                    sql_text = sql_text + " and a.nzp_area = " + nzp_area;
                }

                sql_text = sql_text + " Group by a.nzp_payer, a.nzp_area, a.nzp_dom,a.nzp_serv, a.nzp_bank ";
                ret = ExecSQL(conn_db, transaction, sql_text, true);
                if (!ret.result)
                {
                    return;
                ...
                " Set sum_in = (select a.sum_in from ttt_dis_prev  a where " + 
                fn_packXX.fn_distrib + ".nzp_serv = a.nzp_serv and " +
                fn_packXX.fn_distrib + ".nzp_dom = a.nzp_dom and " +
                fn_packXX.fn_distrib + ".nzp_area = a.nzp_area and " +
                fn_packXX.fn_distrib + ".nzp_payer = a.nzp_payer and " +
                fn_packXX.fn_distrib + ".nzp_bank = a.nzp_bank  " +
                ") "+
                " Where dat_oper = '" + dat.ToShortDateString()+"'" + sConvToDate;
                if (nzp_payer>0)
                {
                    sql_text = sql_text + " and nzp_payer = " + nzp_payer;
                }
                if (nzp_area > 0)
                {
                    sql_text = sql_text + " and nzp_area = " + nzp_area;
                }
 ...
 strNVL="coalesce(";
                ...
                sql_text = " Update " + fn_packXX.fn_distrib +
                " Set sum_out = " + strNVL + "sum_in,0) + " + strNVL + "sum_rasp,0) - " + strNVL + "sum_ud,0) + " + strNVL + "sum_naud,0) + " + strNVL + "sum_reval,0) - " + strNVL + "sum_send,0) " +
                "   ," +  "sum_charge = " + strNVL + "sum_rasp,0) - " + strNVL + "sum_ud,0) + " + strNVL + "sum_naud,0) + " + strNVL + "sum_reval,0) " +
                " Where dat_oper = '" + dat.ToShortDateString()+"' "+ sConvToDate ;
                if (nzp_payer > 0)
                {
                    sql_text = sql_text + " and nzp_payer = " + nzp_payer;
        ...
        //-----------------------------------------------------------------------------
        public void RecalcUderFndistrib(IDbConnection conn_db,  DateTime min_dat_oper, out Returns ret, DbCalcQueueClient.SetTaskProgressDelegate setTaskProgress)
        //-----------------------------------------------------------------------------
        {
            ret = Utils.InitReturns();
        ...
        //-----------------------------------------------------------------------------
        {
            ...
            MonitorLog.WriteLog("Начало расчёта комиссии за обслуживание за " + dat_oper, MonitorLog.typelog.Info, 1, 222, true);
            ret = Utils.InitReturns();
            string sql_text="";
            string TablePercent;
            if (ptype ==0)
            ...
            if (!ret.result)
            {
                ...
                sql_text = " Update ttt_paxx " +
                    ...
                    " Where 0 < ( Select count(*) From " + fn_packXX.fn_distrib + " a " +
                                ...
                                "   and a.nzp_serv = ttt_paxx.nzp_serv " +
                                "   and a.nzp_bank = ttt_paxx.nzp_bank " +
                                "   and a.dat_oper = " + sDat_oper + sConvToDate + " ) ";
                ret = ExecSQL(conn_db,sql_text , true);
                if (!ret.result)
                {
                    return;
...
ExecSQL(conn_db, " analyze ttt_paxx ", true);
 ...
 ExecSQL(conn_db, " analyze ttt_perc_ud ", true);
  ...
  ret = ExecSQL(conn_db,
            ...
            sql_text = "select * from  " + TablePercent + " where nzp_serv_from >0 and " + sDat_oper + sConvToDate + " between dat_s and dat_po";
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    IDataReader reader2;
                    sql_text = "select p.nzp_payer, s.nzp_supp from " + Points.Pref + "_data" + tableDelimiter + "s_area s, " + Points.Pref + "_kernel" + tableDelimiter + "s_payer p where   s.nzp_area  = " + nzp_area + " and s.nzp_supp = p.nzp_supp ";
                    ...
                    reader2.Read();
                    int nzp_payer_from = (int)reader2["nzp_payer"];
                    int nzp_supp_from = (int)reader2["nzp_supp"];
                    reader2.Close();

                    if ((nzp_payer_from > 0) & (nzp_area > 0))
                    {
                        ...
                        sql_text = "create " + sCrtTempTable + " table t_ud_from_uk2 (nzp_area integer, nzp_payer integer, nzp_dom integer, nzp_serv integer, sum_prih " + DBManager.sDecimalType + "(20,5) default 0 ,sum_ud " + DBManager.sDecimalType + "(20,5) default 0, nzp_bank integer) " + sUnlogTempTable;
                        ret = ExecSQL(conn_db, sql_text, true);
                        if (!ret.result)
                        {
                            MonitorLog.WriteLog(ret.text, MonitorLog.typelog.Error, 1, 222, true);
                        ...
                        //if (!ret.result)
                        //{
                        //    ret = ExecSQL(conn_db, "delete from t_ud_from_uk ", false);
                       // }
                       // else
                       // {
...
//                        }
                        ...
                        sql_text = " insert into t_ud_from_uk2(nzp_area , nzp_payer, nzp_dom, nzp_serv, nzp_bank,  sum_prih) " +
                            " Select nzp_area, nzp_payer, nzp_dom, nzp_serv,nzp_bank,sum(sum_rasp) sum_prih From  " + fn_packXX.fn_distrib +
                            " Where  nzp_area = " + nzp_area + " and nzp_dom in (select nzp_dom from t_dom) and dat_oper = "+sDat_oper + sConvToDate;

                        if (nzp_supp > 0)
                        {
                            sql_text = sql_text + "   and nzp_supp = " + nzp_supp;
                        }
                        if (nzp_serv > 0)
                        {
                            sql_text = sql_text + "   and nzp_serv = " + nzp_serv;
                        }
                        if (nzp_bank > 0)
                        {
                        ...
                        //if (isDebug)
                        //    count = 1;
                        //ExecSQL(conn_db, " Create index idz_1_t_ud_from_uk on t_ud_from_uk (nzp_payer,nzp_dom) ", true);
                        if (count > 0)
                        {
                            sql_text = " update  t_ud_from_uk2 set sum_ud = sum_prih * (" + perc_ud + "/100) ";
                            ret = ExecSQL(conn_db, sql_text, true);
                            if (!ret.result)
                            {
                                MonitorLog.WriteLog(ret.text, MonitorLog.typelog.Error, 1, 222, true);
                            ...
                            // Удержаннаые суммы
                            sql_text = " Insert into  " + TmpTablePref + "ttt_perc_ud  (nzp_area,nzp_payer,nzp_payer_naud,nzp_supp, nzp_serv, nzp_dom, nzp_bank, perc_ud, sum_prih,  sum_naud) " +
                                " Select  nzp_area, " + nzp_payer_from + "," + nzp_payer_to + "," + nzp_supp_from + "," + nzp_serv_from + ",nzp_dom,nzp_bank, " + perc_ud + ", SUM(sum_prih)" + ",  SUM(sum_ud) " +
                                " From t_ud_from_uk2" +
                                "  group by 1,2,3,4,5,6,7,8 having  SUM(sum_ud)>0";
                            ...
                            /*
                            // Начисленные за обслуживание

                            sql_text = " Insert into  " + TmpTablePref + " ttt_perc_ud  (nzp_area,nzp_payer, nzp_payer_naud,nzp_serv, nzp_dom, nzp_bank, perc_ud, sum_prih,  sum_naud) " +
                                " Select  nzp_area, " + nzp_payer_from+ ","+nzp_payer_to + "," + nzp_serv_from + ",nzp_dom,-1 nzp_bank, " + perc_ud + ", SUM(sum_prih)" + ",  SUM(sum_ud) " +
                                " From t_ud_from_uk" +
                                " Where abs(sum_ud) > 0.001  group by 1,2,3,4,5,6";
            ...
            sql_text=" Insert into " + fn_packXX.fn_perc + " (nzp_supp, nzp_payer, nzp_dom, nzp_serv, nzp_area, nzp_geu, sum_prih, sum_perc, perc_ud, dat_oper, nzp_bank) " +
                " Select nzp_supp, nzp_payer_naud, nzp_dom,nzp_serv, nzp_area, -1 nzp_geu, sum_prih, sum_naud, perc_ud, " + sDat_oper+  sConvToDate + ", nzp_bank " +
                " From ttt_perc_ud " +
                " Where abs(sum_naud) > 0.001 ";
            ret = ExecSQL(conn_db,sql_text, true);
            if (!ret.result)
            ...
            sql_text = " Insert into " + fn_packXX.fn_naud + " (dat_oper, nzp_payer, nzp_dom, nzp_serv, nzp_payer_2, sum_prih, perc_ud, sum_ud, sum_naud, nzp_area, nzp_geu, nzp_bank) " +
                " Select " + sDat_oper + sConvToDate + ", nzp_payer, nzp_dom, nzp_serv, nzp_payer_naud, sum_prih, perc_ud, sum_naud, 0 as sum_naud, nzp_area, 0 nzp_geu, nzp_bank " +
                " From ttt_perc_ud " +
                " Where abs(sum_naud) > 0.001 ";
            ret = ExecSQL(conn_db,sql_text , true);
            if (!ret.result)
            ...
            sql_text = " Insert into " + fn_packXX.fn_naud + " (dat_oper, nzp_payer, nzp_dom,nzp_serv, nzp_payer_2, sum_prih, perc_ud, sum_ud, sum_naud, nzp_area, nzp_geu, nzp_bank) " +
                " Select " + sDat_oper + sConvToDate + ", nzp_payer_naud, nzp_dom, nzp_serv, nzp_payer,sum_prih, perc_ud, 0 sum_naud, sum_naud, nzp_area, 0 nzp_geu, nzp_bank " +
                " From ttt_perc_ud " +
                " Where abs(sum_naud) > 0.001 ";
            ret = ExecSQL(conn_db,sql_text , true);
            if (!ret.result)
 ...
 ExecSQL(conn_db, " analyze ttt_perc_ud ", true);
            ...
            sql_text=
                " Insert into " + fn_packXX.fn_distrib + " ( nzp_payer,nzp_area, nzp_dom,nzp_serv,nzp_bank,dat_oper ) " +
#if PG
                " Select distinct nzp_payer_naud,nzp_area, nzp_dom,nzp_serv,nzp_bank, to_date(" + sDat_oper + ", 'dd.mm.yyyy')" +
#else
 " Select unique nzp_payer_naud,nzp_area, nzp_dom,nzp_serv,nzp_bank, " + sDat_oper +
#endif
 " From ttt_perc_ud Where kod = 1 ";
            ...
            MonitorLog.WriteLog("Завершение расчёта комиссии за обслуживание за " + dat_oper, MonitorLog.typelog.Info, 1, 222, true);
        }
